...
public class VillageSiege implements ISpecialSpawner {
   ...
   public int onUpdate(ServerWorld world, boolean spawnHostiles, boolean spawnPassives) {
      if (!world.isDaytime() && spawnHostiles) {
         float f = world.func_242415_f(0.0F);
         if ((double)f == 0.5D) {
            this.siegeState = world.rand.nextInt(10) == 0 ? VillageSiege.State.SIEGE_TONIGHT : VillageSiege.State.SIEGE_DONE;
         }
         ...
         if (!playerentity.isSpectator()) {
            ...
            if (world.isVillage(blockpos) && world.getBiome(blockpos).getCategory() != Biome.Category.MUSHROOM) {
               for(int i = 0; i < 10; ++i) {
                  ...
                  Vector3d siegeLocation = this.findRandomSpawnPos(world, new BlockPos(this.spawnX, this.spawnY, this.spawnZ));
