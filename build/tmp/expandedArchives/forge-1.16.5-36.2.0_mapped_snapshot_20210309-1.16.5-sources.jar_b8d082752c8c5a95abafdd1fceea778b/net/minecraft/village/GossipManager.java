...
public class GossipManager {
   private final Map<UUID, GossipManager.Gossips> uniqueGossipsMap = Maps.newHashMap();

   public void tick() {
      Iterator<GossipManager.Gossips> iterator = this.uniqueGossipsMap.values().iterator();
   ...
   private Stream<GossipManager.GossipEntry> getGossipEntries() {
      return this.uniqueGossipsMap.entrySet().stream().flatMap((uniqueGossipEntry) -> {
   ...
   private GossipManager.Gossips getOrCreate(UUID identifier) {
      return this.uniqueGossipsMap.computeIfAbsent(identifier, (id) -> {
   ...
   public int getReputation(UUID identifier, Predicate<GossipType> gossip) {
      GossipManager.Gossips gossipmanager$gossips = this.uniqueGossipsMap.get(identifier);
   ...
   public void add(UUID identifier, GossipType gossipType, int gossipValue) {
      ...
      if (gossipmanager$gossips.isGossipTypeMapEmpty()) {
         this.uniqueGossipsMap.remove(identifier);
      }
