...
public class ClientWorld extends World {
   ...
   public ClientWorld(ClientPlayNetHandler p_i242067_1_, ClientWorld.ClientWorldInfo p_i242067_2_, RegistryKey<World> p_i242067_3_, DimensionType p_i242067_4_, int p_i242067_5_, Supplier<IProfiler> p_i242067_6_, WorldRenderer p_i242067_7_, boolean p_i242067_8_, long p_i242067_9_) {
      ...
      this.chunkProvider = new ClientChunkProvider(this, p_i242067_5_);
      this.clientWorldInfo = p_i242067_2_;
      this.worldRenderer = p_i242067_7_;
      this.dimensionRenderInfo = DimensionRenderInfo.func_243495_a(p_i242067_4_);
      this.setSpawnPosition(new BlockPos(8, 64, 8), 0.0F);
   ...
   public DimensionRenderInfo getDimensionRenderInfo() {
   ...
   public void setGameTime(long time) {
      this.clientWorldInfo.setGameTime(time);
   }
   ...
   public void updateEntityRidden(Entity mount, Entity rider) {
      if (!rider.removed && rider.getRidingEntity() == mount) {
         if (rider instanceof PlayerEntity || this.getChunkProvider().isChunkLoaded(rider)) {
            rider.forceSetPosition(rider.getPosX(), rider.getPosY(), rider.getPosZ());
            rider.prevRotationYaw = rider.rotationYaw;
            rider.prevRotationPitch = rider.rotationPitch;
            if (rider.addedToChunk) {
               ++rider.ticksExisted;
               rider.updateRidden();
            }

            this.checkChunk(rider);
            if (rider.addedToChunk) {
               for(Entity entity : rider.getPassengers()) {
                  this.updateEntityRidden(rider, entity);
               }
            }
   ...
   public void animateTick(int x, int y, int z, int offset, Random random, boolean holdingBarrier, BlockPos.Mutable pos) {
      ...
      if (holdingBarrier && blockstate.matchesBlock(Blocks.BARRIER)) {
   ...
   private void spawnFluidParticle(BlockPos blockPosIn, BlockState blockStateIn, IParticleData particleDataIn, boolean shapeDownSolid) {
      if (blockStateIn.getFluidState().isEmpty()) {
         VoxelShape voxelshape = blockStateIn.getCollisionShapeUncached(this, blockPosIn);
         ...
         if (d0 < 1.0D) {
            ...
            if (d1 > 0.0D) {
               ...
               VoxelShape voxelshape1 = blockstate.getCollisionShapeUncached(this, blockpos);
   ...
   public DynamicRegistries func_241828_r() {
      return this.connection.getRegistries();
   }
   ...
   public float func_230487_a_(Direction p_230487_1_, boolean p_230487_2_) {
      boolean flag = this.getDimensionRenderInfo().func_239217_c_();
      if (!p_230487_2_) {
   ...
   public void setSpawnPosition(BlockPos spawnPos, float p_239136_2_) {
      this.worldInfo.setSpawn(spawnPos, p_239136_2_);
   }
   ...
   public static class ClientWorldInfo implements ISpawnWorldInfo {
      ...
      public ClientWorldInfo(Difficulty difficulty, boolean hardcore, boolean flatWorld) {
