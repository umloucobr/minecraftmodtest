...
public class PlayerController {
   ...
   private GameType previousGameType = GameType.NOT_SET;
   ...
   public void setPreviousGameType(GameType gameType) {
   ...
   public ActionResultType func_217292_a(ClientPlayerEntity player, ClientWorld world, Hand hand, BlockRayTraceResult p_217292_4_) {
      this.syncCurrentPlayItem();
      ...
      if (!this.mc.world.getWorldBorder().contains(blockpos)) {
         ...
         net.minecraftforge.event.entity.player.PlayerInteractEvent.RightClickBlock event = net.minecraftforge.common.ForgeHooks
                 .onRightClickBlock(player, hand, blockpos, p_217292_4_);
         if (event.isCanceled()) {
         ...
         if (this.currentGameType == GameType.SPECTATOR) {
            ...
            ItemUseContext itemusecontext = new ItemUseContext(player, hand, p_217292_4_);
            ...
            if (event.getUseBlock() == net.minecraftforge.eventbus.api.Event.Result.ALLOW || (event.getUseBlock() != net.minecraftforge.eventbus.api.Event.Result.DENY && !flag1)) {
               ActionResultType actionresulttype = world.getBlockState(blockpos).onBlockActivated(world, player, hand, p_217292_4_);
            ...
            if (event.getUseItem() == net.minecraftforge.eventbus.api.Event.Result.ALLOW || (!itemstack.isEmpty() && !player.getCooldownTracker().hasCooldown(itemstack.getItem()))) {
   ...
   public ClientPlayerEntity createPlayer(ClientWorld world, StatisticsManager statsManager, ClientRecipeBook recipes, boolean p_239167_4_, boolean p_239167_5_) {
      return new ClientPlayerEntity(this.mc, world, this.connection, statsManager, recipes, p_239167_4_, p_239167_5_);
   }
   ...
   public void sendPlaceRecipePacket(int p_203413_1_, IRecipe<?> recipe, boolean placeAll) {
      this.connection.sendPacket(new CPlaceRecipePacket(p_203413_1_, recipe, placeAll));
   }
   ...
   public void acknowledgePlayerDiggingReceived(ClientWorld worldIn, BlockPos pos, BlockState blockIn, CPlayerDiggingPacket.Action action, boolean successful) {
      ...
      if ((vector3d == null || !successful || action != CPlayerDiggingPacket.Action.START_DESTROY_BLOCK && blockstate != blockIn) && blockstate != blockIn) {
         ...
         if (vector3d != null && worldIn == playerentity.world && playerentity.func_242278_a(pos, blockIn)) {
            playerentity.setPositionWithinBounds(vector3d.x, vector3d.y, vector3d.z);
         }
      }
