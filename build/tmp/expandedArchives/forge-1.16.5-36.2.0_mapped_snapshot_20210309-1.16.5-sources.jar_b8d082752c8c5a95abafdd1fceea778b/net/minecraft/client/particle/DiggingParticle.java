...
public class DiggingParticle extends SpriteTexturedParticle {
   ...
   public DiggingParticle setBlockPos(BlockPos pos) {
      updateSprite(pos);
      this.sourcePos = pos;
      if (this.sourceState.matchesBlock(Blocks.GRASS_BLOCK)) {
         return this;
      } else {
   ...
   public static class Factory implements IParticleFactory<BlockParticleData> {
      public Particle makeParticle(BlockParticleData typeIn, ClientWorld worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
         ...
         return !blockstate.isAir() && !blockstate.matchesBlock(Blocks.MOVING_PISTON) ? (new DiggingParticle(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, blockstate)).init().updateSprite(typeIn.getPos()) : null;
      }
   }
