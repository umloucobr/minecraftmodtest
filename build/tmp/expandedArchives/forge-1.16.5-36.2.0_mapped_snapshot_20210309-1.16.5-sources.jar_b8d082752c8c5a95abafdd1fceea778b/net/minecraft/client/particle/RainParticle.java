...
public class RainParticle extends SpriteTexturedParticle {
   ...
   public void tick() {
      ...
      if (this.maxAge-- <= 0) {
         ...
         double d0 = Math.max(this.world.getBlockState(blockpos).getCollisionShapeUncached(this.world, blockpos).max(Direction.Axis.Y, this.posX - (double)blockpos.getX(), this.posZ - (double)blockpos.getZ()), (double)this.world.getFluidState(blockpos).getActualHeight(this.world, blockpos));
