...
public class LanguageScreen extends SettingsScreen {
   private static final ITextComponent field_243292_c = (new StringTextComponent("(")).appendSibling(new TranslationTextComponent("options.languageWarning")).appendString(")").mergeStyle(TextFormatting.GRAY);
   ...
   class List extends ExtendedList<LanguageScreen.List.LanguageEntry> {
      ...
      public class LanguageEntry extends ExtendedList.AbstractListEntry<LanguageScreen.List.LanguageEntry> {
         ...
         public void render(MatrixStack matrixStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean isMouseOver, float partialTicks) {
            ...
            LanguageScreen.this.font.drawStringWithTransparency(matrixStack, s, (float)(List.this.width / 2 - LanguageScreen.this.font.getStringWidth(s) / 2), (float)(top + 1), 16777215, true);
         }
