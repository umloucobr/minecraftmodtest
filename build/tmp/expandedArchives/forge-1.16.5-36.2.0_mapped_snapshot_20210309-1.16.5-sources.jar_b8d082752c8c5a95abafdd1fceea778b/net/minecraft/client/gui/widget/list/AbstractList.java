...
public abstract class AbstractList<E extends AbstractList.AbstractListEntry<E>> extends FocusableGui implements IRenderable {
   ...
   private final List<E> entries = new AbstractList.SimpleArrayList();
   protected int width;
   ...
   protected final E getEntryAtPosition(double mouseX, double mouseY) {
      ...
      return (E)(mouseX < (double)this.getScrollbarPosition() && mouseX >= (double)k && mouseX <= (double)l && j1 >= 0 && i1 >= 0 && j1 < this.getItemCount() ? this.getEventListeners().get(j1) : null);
   }

   public void updateSize(int width, int height, int p_230940_3_, int p_230940_4_) {
      this.width = width;
      this.height = height;
      this.y0 = p_230940_3_;
      this.y1 = p_230940_4_;
   ...
   protected void centerScrollOn(E entry) {
      this.setScrollAmount((double)(this.getEventListeners().indexOf(entry) * this.itemHeight + this.itemHeight / 2 - (this.y1 - this.y0) / 2));
   }

   protected void ensureVisible(E entry) {
      int i = this.getRowTop(this.getEventListeners().indexOf(entry));
   ...
   public void setScrollAmount(double scroll) {
      this.scrollAmount = MathHelper.clamp(scroll, 0.0D, (double)this.getMaxScroll());
   }
   ...
   protected void moveSelection(AbstractList.Ordering ordering) {
      this.filterList(ordering, (entry) -> {
         return true;
      });
   }
   ...
   protected void filterList(AbstractList.Ordering ordering, Predicate<E> filter) {
      int i = ordering == AbstractList.Ordering.UP ? -1 : 1;
   ...
   protected void renderList(MatrixStack matrixStack, int x, int y, int mouseX, int mouseY, float partialTicks) {
      ...
      for(int j = 0; j < i; ++j) {
         ...
         if (l >= this.y0 && k <= this.y1) {
            ...
            e.render(matrixStack, j, k, j2, k1, j1, mouseX, mouseY, this.isMouseOver((double)mouseX, (double)mouseY) && Objects.equals(this.getEntryAtPosition((double)mouseX, (double)mouseY), e), partialTicks);
         }
      }
   ...
   protected E remove(int index) {
      E e = this.entries.get(index);
      return (E)(this.removeEntry(this.entries.get(index)) ? e : null);
   }

   protected boolean removeEntry(E entry) {
      boolean flag = this.entries.remove(entry);
   ...
   private void func_238480_f_(AbstractList.AbstractListEntry<E> entry) {
      entry.list = this;
   }
   ...
   public abstract static class AbstractListEntry<E extends AbstractList.AbstractListEntry<E>> implements IGuiEventListener {
      ...
      public abstract void render(MatrixStack matrixStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean isMouseOver, float partialTicks);
