   ...
   class DetailsList extends ExtendedList<CreateFlatWorldScreen.DetailsList.LayerEntry> {
      ...
      class LayerEntry extends ExtendedList.AbstractListEntry<CreateFlatWorldScreen.DetailsList.LayerEntry> {
         ...
         public void render(MatrixStack matrixStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean isMouseOver, float partialTicks) {
            FlatLayerInfo flatlayerinfo = CreateFlatWorldScreen.this.generatorInfo.getFlatLayers().get(CreateFlatWorldScreen.this.generatorInfo.getFlatLayers().size() - index - 1);
            BlockState blockstate = flatlayerinfo.getLayerMaterial();
            Item item = blockstate.getBlock().asItem();
            if (item == Items.AIR) {
               if (blockstate.matchesBlock(Blocks.WATER)) {
                  item = Items.WATER_BUCKET;
               } else if (blockstate.matchesBlock(Blocks.LAVA)) {
                  item = Items.LAVA_BUCKET;
               }
            }

            ItemStack itemstack = new ItemStack(item);
            this.func_238605_a_(matrixStack, left, top, itemstack);
            CreateFlatWorldScreen.this.font.drawText(matrixStack, item.getDisplayName(itemstack), (float)(left + 18 + 5), (float)(top + 3), 16777215);
            String s;
            if (index == 0) {
               s = I18n.format("createWorld.customize.flat.layer.top", flatlayerinfo.getLayerCount());
            } else if (index == CreateFlatWorldScreen.this.generatorInfo.getFlatLayers().size() - 1) {
            ...
            CreateFlatWorldScreen.this.font.drawString(matrixStack, s, (float)(left + 2 + 213 - CreateFlatWorldScreen.this.font.getStringWidth(s)), (float)(top + 3), 16777215);
         }
