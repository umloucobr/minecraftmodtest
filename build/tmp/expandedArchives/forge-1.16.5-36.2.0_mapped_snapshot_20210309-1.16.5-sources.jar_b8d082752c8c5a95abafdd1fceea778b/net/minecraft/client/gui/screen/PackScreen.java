...
public class PackScreen extends Screen {
      ...
      p_238895_1_.forEach((p_238901_2_) -> {
         try (Stream<Path> stream = Files.walk(p_238901_2_)) {
            stream.forEach((p_238900_3_) -> {
               try {
                  Util.moveFileToDirectory(p_238901_2_.getParent(), p_238895_2_, p_238900_3_);
   ...
   private ResourceLocation func_243397_a(TextureManager p_243397_1_, ResourcePackInfo p_243397_2_) {
      try (
         ...
         ResourceLocation resourcelocation = new ResourceLocation("minecraft", "pack/" + Util.validateCharacters(s, ResourceLocation::validatePathChar) + "/" + Hashing.sha1().hashUnencodedChars(s) + "/icon");
