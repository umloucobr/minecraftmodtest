...
public class ServerSelectionList extends ExtendedList<ServerSelectionList.Entry> {
   ...
   protected void moveSelection(AbstractList.Ordering ordering) {
      this.filterList(ordering, (p_241612_0_) -> {
   ...
   public static class LanDetectedEntry extends ServerSelectionList.Entry {
      ...
      public void render(MatrixStack matrixStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean isMouseOver, float partialTicks) {
         this.mc.fontRenderer.drawText(matrixStack, field_243386_c, (float)(left + 32 + 3), (float)(top + 1), 16777215);
         this.mc.fontRenderer.drawString(matrixStack, this.serverData.getServerMotd(), (float)(left + 32 + 3), (float)(top + 12), 8421504);
         if (this.mc.gameSettings.hideServerAddress) {
            this.mc.fontRenderer.drawText(matrixStack, field_243387_d, (float)(left + 32 + 3), (float)(top + 12 + 11), 3158064);
         } else {
            this.mc.fontRenderer.drawString(matrixStack, this.serverData.getServerIpPort(), (float)(left + 32 + 3), (float)(top + 12 + 11), 3158064);
         }
   ...
   public static class LanScanEntry extends ServerSelectionList.Entry {
      ...
      public void render(MatrixStack matrixStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean isMouseOver, float partialTicks) {
         ...
         this.mc.fontRenderer.drawText(matrixStack, ServerSelectionList.field_243365_r, (float)(this.mc.currentScreen.width / 2 - this.mc.fontRenderer.getStringPropertyWidth(ServerSelectionList.field_243365_r) / 2), (float)i, 16777215);
         String s;
         ...
         this.mc.fontRenderer.drawString(matrixStack, s, (float)(this.mc.currentScreen.width / 2 - this.mc.fontRenderer.getStringWidth(s) / 2), (float)(i + 9), 8421504);
      }
   }
   ...
   public class NormalEntry extends ServerSelectionList.Entry {
      ...
      public void render(MatrixStack matrixStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean isMouseOver, float partialTicks) {
         ...
         this.mc.fontRenderer.drawString(matrixStack, this.server.serverName, (float)(left + 32 + 3), (float)(top + 1), 16777215);
         List<IReorderingProcessor> list = this.mc.fontRenderer.trimStringToWidth(this.server.serverMOTD, width - 32 - 2);

         for(int i = 0; i < Math.min(list.size(), 2); ++i) {
            this.mc.fontRenderer.func_238422_b_(matrixStack, list.get(i), (float)(left + 32 + 3), (float)(top + 12 + 9 * i), 8421504);
         }
         ...
         this.mc.fontRenderer.drawText(matrixStack, itextcomponent1, (float)(left + width - j - 15 - 2), (float)(top + 1), 8421504);
         int k = 0;
         int l;
         ...
         if (flag) {
            ...
            k = 1;
            l = (int)(Util.milliTime() / 100L + (long)(index * 2) & 7L);
            if (l > 4) {
               l = 8 - l;
            }
         ...
         AbstractGui.blit(matrixStack, left + width - 15, top, (float)(k * 10), (float)(176 + l * 8), 10, 8, 256, 256);
         ...
         if (this.icon != null) {
            ...
            this.func_238859_a_(matrixStack, left, top, ServerSelectionList.field_214359_c);
         }
         ...
         net.minecraftforge.fml.client.ClientHooks.drawForgePingInfo(this.owner, server, matrixStack, left, top, width, i1, j1);

         if (this.mc.gameSettings.touchscreen || isMouseOver) {
