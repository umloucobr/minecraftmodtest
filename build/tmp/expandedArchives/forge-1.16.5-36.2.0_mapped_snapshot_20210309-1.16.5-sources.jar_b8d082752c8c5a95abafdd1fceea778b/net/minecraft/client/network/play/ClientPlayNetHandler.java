...
public class ClientPlayNetHandler implements IClientPlayNetHandler {
   ...
   private ClientWorld.ClientWorldInfo worldInfo;
   /**
   ...
   private DynamicRegistries dynamicRegistries = DynamicRegistries.func_239770_b_();
   ...
   public void handleJoinGame(SJoinGamePacket packetIn) {
      ...
      ArrayList<RegistryKey<World>> arraylist = Lists.newArrayList(packetIn.getDimensionKeys());
      Collections.shuffle(arraylist);
      this.dimensionKeys = Sets.newLinkedHashSet(arraylist);
      this.dynamicRegistries = packetIn.getDynamicRegistries();
      RegistryKey<World> registrykey = packetIn.getDimension();
      DimensionType dimensiontype = packetIn.getSpawnDimension();
      this.viewDistance = packetIn.getViewDistance();
      boolean flag = packetIn.func_240820_n_();
      boolean flag1 = packetIn.isFlatWorld();
      ...
      this.worldInfo = clientworld$clientworldinfo;
      ...
      this.client.player.setShowDeathScreen(packetIn.shouldEnableRespawnScreen());
      this.client.playerController.setGameType(packetIn.getGameType());
      this.client.playerController.setPreviousGameType(packetIn.getPreviousGameType());
   ...
   public void handleSpawnObject(SSpawnObjectPacket packetIn) {
      ...
      if (entitytype == EntityType.CHEST_MINECART) {
         ...
         entity = new LlamaSpitEntity(this.world, d0, d1, d2, packetIn.getAccelerationX(), packetIn.getAccelerationY(), packetIn.getAccelerationZ());
         ...
         entity = new FireballEntity(this.world, d0, d1, d2, packetIn.getAccelerationX(), packetIn.getAccelerationY(), packetIn.getAccelerationZ());
      } else if (entitytype == EntityType.DRAGON_FIREBALL) {
         entity = new DragonFireballEntity(this.world, d0, d1, d2, packetIn.getAccelerationX(), packetIn.getAccelerationY(), packetIn.getAccelerationZ());
      } else if (entitytype == EntityType.SMALL_FIREBALL) {
         entity = new SmallFireballEntity(this.world, d0, d1, d2, packetIn.getAccelerationX(), packetIn.getAccelerationY(), packetIn.getAccelerationZ());
      } else if (entitytype == EntityType.WITHER_SKULL) {
         entity = new WitherSkullEntity(this.world, d0, d1, d2, packetIn.getAccelerationX(), packetIn.getAccelerationY(), packetIn.getAccelerationZ());
      } else if (entitytype == EntityType.SHULKER_BULLET) {
         entity = new ShulkerBulletEntity(this.world, d0, d1, d2, packetIn.getAccelerationX(), packetIn.getAccelerationY(), packetIn.getAccelerationZ());
   ...
   public void handleEntityMovement(SEntityPacket packetIn) {
      ...
      if (entity != null) {
         if (!entity.canPassengerSteer()) {
            if (packetIn.isMovePacket()) {
               Vector3d vector3d = packetIn.func_244300_a(entity.positionOffset());
   ...
   public void handleMultiBlockChange(SMultiBlockChangePacket packetIn) {
      ...
      packetIn.alterBlock((pos, state) -> {
         this.world.setBlockState(pos, state, i);
      });
   }
   ...
   public void handleChunkData(SChunkDataPacket packetIn) {
      ...
      BiomeContainer biomecontainer = packetIn.getBiomeIds() == null ? null : new BiomeContainer(this.dynamicRegistries.getRegistry(Registry.BIOME_KEY), packetIn.getBiomeIds());
   ...
   public void handleChat(SChatPacket packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
      net.minecraft.util.text.ITextComponent message = net.minecraftforge.event.ForgeEventFactory.onClientChat(packetIn.getType(), packetIn.getChatComponent(), packetIn.getSender());
      if (message == null) return;
      this.client.ingameGUI.sendChatMessage(packetIn.getType(), message, packetIn.getSender());
   }
   ...
   public void func_230488_a_(SWorldSpawnChangedPacket packet) {
      PacketThreadUtil.checkThreadAndEnqueue(packet, this, this.client);
      this.client.world.setSpawnPosition(packet.getSpawnPos(), packet.getSpawnAngle());
   }
   ...
   public void handleRespawn(SRespawnPacket packetIn) {
      ...
      if (registrykey != clientplayerentity.world.getDimensionKey()) {
         ...
         boolean flag1 = packetIn.isFlatWorld();
         ClientWorld.ClientWorldInfo clientworld$clientworldinfo = new ClientWorld.ClientWorldInfo(this.worldInfo.getDifficulty(), this.worldInfo.isHardcore(), flag1);
         this.worldInfo = clientworld$clientworldinfo;
         this.world = new ClientWorld(this, clientworld$clientworldinfo, registrykey, dimensiontype, this.viewDistance, this.client::getProfiler, this.client.worldRenderer, flag, packetIn.getHashedSeed());
   ...
   public void handleOpenHorseWindow(SOpenHorseWindowPacket packetIn) {
      ...
      if (entity instanceof AbstractHorseEntity) {
         ...
         HorseInventoryContainer horseinventorycontainer = new HorseInventoryContainer(packetIn.getWindowId(), clientplayerentity.inventory, inventory, abstracthorseentity);
   ...
   public void handleEntityEquipment(SEntityEquipmentPacket packetIn) {
      ...
      Entity entity = this.world.getEntityByID(packetIn.getEntityID());
      if (entity != null) {
         packetIn.getEquipped().forEach((slotStackPair) -> {
            entity.setItemStackToSlot(slotStackPair.getFirst(), slotStackPair.getSecond());
         });
      }
   ...
   public void handleChangeGameState(SChangeGameStatePacket packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
      PlayerEntity playerentity = this.client.player;
      SChangeGameStatePacket.State schangegamestatepacket$state = packetIn.getState();
      float f = packetIn.getValue();
      int i = MathHelper.floor(f + 0.5F);
      if (schangegamestatepacket$state == SChangeGameStatePacket.SPAWN_NOT_VALID) {
         playerentity.sendStatusMessage(new TranslationTextComponent("block.minecraft.spawn.not_valid"), false);
      } else if (schangegamestatepacket$state == SChangeGameStatePacket.RAINING) {
         this.world.getWorldInfo().setRaining(true);
         this.world.setRainStrength(0.0F);
      } else if (schangegamestatepacket$state == SChangeGameStatePacket.CLEAR) {
         this.world.getWorldInfo().setRaining(false);
         this.world.setRainStrength(1.0F);
      } else if (schangegamestatepacket$state == SChangeGameStatePacket.CHANGE_GAMETYPE) {
         this.client.playerController.setGameType(GameType.getByID(i));
      } else if (schangegamestatepacket$state == SChangeGameStatePacket.PERFORM_RESPAWN) {
         if (i == 0) {
      ...
      } else if (schangegamestatepacket$state == SChangeGameStatePacket.CHANGE_SETTINGS) {
      ...
      } else if (schangegamestatepacket$state == SChangeGameStatePacket.HIT_PLAYER_ARROW) {
      ...
      } else if (schangegamestatepacket$state == SChangeGameStatePacket.SET_RAIN_STRENGTH) {
         this.world.setRainStrength(f);
      } else if (schangegamestatepacket$state == SChangeGameStatePacket.SET_THUNDER_STRENGTH) {
         this.world.setThunderStrength(f);
      } else if (schangegamestatepacket$state == SChangeGameStatePacket.STING_PLAYER_PUFFERFISH) {
      ...
      } else if (schangegamestatepacket$state == SChangeGameStatePacket.CURSE_PLAYER_ELDER_GUARDIAN) {
      ...
      } else if (schangegamestatepacket$state == SChangeGameStatePacket.SHOW_DEATH_SCREEN) {
   ...
   public void handleRecipeBook(SRecipeBookPacket packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
      ClientRecipeBook clientrecipebook = this.client.player.getRecipeBook();
      clientrecipebook.func_242140_a(packetIn.getBookStatus());
      ...
      case ADD:
         for(ResourceLocation resourcelocation : packetIn.getRecipes()) {
            this.recipeManager.getRecipe(resourcelocation).ifPresent((recipe) -> {
               clientrecipebook.unlock(recipe);
               clientrecipebook.markNew(recipe);
               RecipeToast.addOrUpdate(this.client.getToastGui(), recipe);
            });
         }
      }

      clientrecipebook.getRecipes().forEach((recipeList) -> {
   ...
   public void handleServerDifficulty(SServerDifficultyPacket packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
      this.worldInfo.setDifficulty(packetIn.getDifficulty());
      this.worldInfo.setDifficultyLocked(packetIn.isDifficultyLocked());
   }
   ...
   public void handleTitle(STitlePacket packetIn) {
      ...
      case ACTIONBAR:
         ...
         return;
      case RESET:
         this.client.ingameGUI.renderTitles((ITextComponent)null, (ITextComponent)null, -1, -1, -1);
         this.client.ingameGUI.setDefaultTitlesTimes();
         return;
      }

      this.client.ingameGUI.renderTitles(itextcomponent, itextcomponent1, packetIn.getFadeInTime(), packetIn.getDisplayTime(), packetIn.getFadeOutTime());
   }
   ...
   public void handleSpawnMovingSoundEffect(SSpawnMovingSoundEffectPacket packetIn) {
      ...
      if (entity != null) {
         this.client.world.playMovingSound(this.client.player, entity, packetIn.getSound(), packetIn.getCategory(), packetIn.getVolume(), packetIn.getPitch());
      }
   }
   ...
   public void handleCustomPayload(SCustomPayloadPlayPacket packetIn) {
      ...
      try {
         ...
         if (SCustomPayloadPlayPacket.BRAND.equals(resourcelocation)) {
            ...
            DimensionType dimensiontype = this.dynamicRegistries.func_230520_a_().getOrDefault(packetbuffer.readResourceLocation());
   ...
   public void handleScoreboardObjective(SScoreboardObjectivePacket packetIn) {
      ...
      if (packetIn.getAction() == 0) {
         ...
         ScoreObjective scoreobjective = scoreboard.getObjectiveNullable(s);
   ...
   public void handleUpdateScore(SUpdateScorePacket packetIn) {
      ...
      case CHANGE:
         ...
         break;
      case REMOVE:
         scoreboard.removeObjectiveFromEntity(packetIn.getPlayerName(), scoreboard.getObjectiveNullable(s));
      }
   ...
   public void handleDisplayObjective(SDisplayObjectivePacket packetIn) {
      ...
      ScoreObjective scoreobjective = s == null ? null : scoreboard.getObjective(s);
      ...
      if (entity != null) {
         if (!(entity instanceof LivingEntity)) {
            ...
            for(SEntityPropertiesPacket.Snapshot sentitypropertiespacket$snapshot : packetIn.getSnapshots()) {
               ModifiableAttributeInstance modifiableattributeinstance = attributemodifiermanager.createInstanceIfAbsent(sentitypropertiespacket$snapshot.getAttribute());
   ...
   public void handlePlaceGhostRecipe(SPlaceGhostRecipePacket packetIn) {
      ...
      if (container.windowId == packetIn.getWindowId() && container.getCanCraft(this.client.player)) {
         this.recipeManager.getRecipe(packetIn.getRecipeId()).ifPresent((recipe) -> {
            if (this.client.currentScreen instanceof IRecipeShownListener) {
               RecipeBookGui recipebookgui = ((IRecipeShownListener)this.client.currentScreen).getRecipeGui();
               recipebookgui.setupGhostRecipe(recipe, container.inventorySlots);
            }
   ...
   public void handleChunkPositionPacket(SUpdateChunkPositionPacket packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
      this.world.getChunkProvider().setCenter(packetIn.getChunkX(), packetIn.getChunkZ());
   }
