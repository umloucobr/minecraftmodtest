...
public class FogRenderer {
   ...
   public static void updateFogColor(ActiveRenderInfo activeRenderInfoIn, float partialTicks, ClientWorld worldIn, int renderDistanceChunks, float bossColorModifier) {
      ...
      if (fluidstate.isTagged(FluidTags.WATER)) {
         ...
         Vector3d vector3d2 = CubicSampler.func_240807_a_(vector3d1, (x, y, z) -> {
            return worldIn.getDimensionRenderInfo().func_230494_a_(Vector3d.unpack(biomemanager.getBiomeAtPosition(x, y, z).getFogColor()), f12);
         });
         ...
         if (renderDistanceChunks >= 4) {
            ...
            if (f17 > 0.0F) {
               float[] afloat = worldIn.getDimensionRenderInfo().func_230492_a_(worldIn.func_242415_f(partialTicks), partialTicks);
               if (afloat != null) {
                  f17 = f17 * afloat[3];
