...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
   ...
   public Minecraft(GameConfiguration gameConfig) {
      ...
      if (this.gameSettings.overrideHeight > 0 && this.gameSettings.overrideWidth > 0) {
         screensize = new ScreenSize(this.gameSettings.overrideWidth, this.gameSettings.overrideHeight, gameConfig.screenSize.fullscreenWidth, gameConfig.screenSize.fullscreenHeight, gameConfig.screenSize.fullscreen);
      } else {
      ...
      this.fontRenderer = this.fontResourceMananger.getDefaultFontRenderer();
   ...
   void forceUnicodeFont(boolean forced) {
      this.fontResourceMananger.setForcedUnicodeMap(forced ? ImmutableMap.of(DEFAULT_FONT_RENDERER_NAME, UNIFORM_FONT_RENDERER_NAME) : ImmutableMap.of());
   }
   ...
   public void createWorld(String worldName, WorldSettings worldSettings, DynamicRegistries.Impl dynamicRegistriesIn, DimensionGeneratorSettings dimensionGeneratorSettings) {
      this.loadWorld(worldName, dynamicRegistriesIn, (worldStorage) -> {
         ...
         DimensionGeneratorSettings dimensiongeneratorsettings = dataresult.resultOrPartial(Util.prefixString("Error reading worldgen settings after loading data packs: ", LOGGER::error)).orElse(dimensionGeneratorSettings);
   ...
   private void loadWorld(String worldName, DynamicRegistries.Impl dynamicRegistries, Function<SaveFormat.LevelSave, DatapackCodec> levelSaveToDatapackFunction, Function4<SaveFormat.LevelSave, DynamicRegistries.Impl, IResourceManager, DatapackCodec, IServerConfiguration> quadFunction, boolean vanillaOnly, Minecraft.WorldSelectionType selectionType, boolean creating) {
      ...
      try {
         ...
         minecraft$packmanager = creating ? mgr : this.reloadDatapacks(dyn_f, levelSaveToDatapackFunction, quadFunction, vanillaOnly, saveformat$levelsave); //Note this runs the injection again... which isn't good.. but hey.. we need a better spot to hook in.
      ...
      if (selectionType == Minecraft.WorldSelectionType.NONE || !flag && !flag1) {
         ...
         this.deleteWorld(selectionType, worldName, flag, () -> {
            this.loadWorld(worldName, dyn_f, levelSaveToDatapackFunction, quadFunction, vanillaOnly, Minecraft.WorldSelectionType.NONE, creating);
         });
   ...
   private static ResourcePackInfo makePackInfo(String name, boolean isAlwaysEnabled, Supplier<IResourcePack> resourceSupplier, IResourcePack resourcePack, PackMetadataSection resourcePackMeta, ResourcePackInfo.Priority priority, IPackNameDecorator decorator) {
      ...
      return new ResourcePackInfo(name, isAlwaysEnabled, supplier, resourcePack, resourcePackMeta, priority, decorator, resourcePack.isHidden());
   }
