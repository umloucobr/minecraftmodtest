...
public class SoundEngine {
   ...
   private synchronized void load() {
      if (!this.loaded) {
         try {
            this.sndSystem.init();
            this.listener.init();
            this.listener.setVolume(this.options.getSoundLevel(SoundCategory.MASTER));
   ...
   public void play(ISound p_sound) {
      if (this.loaded) {
         ...
         if (p_sound != null && p_sound.shouldPlaySound()) {
                           ...
                           if (!flag3) {
                              this.audioStreamManager.createResource(sound.getSoundAsOggLocation()).thenAccept((audioBuffer) -> {
                                 channelmanager$entry.runOnSoundExecutor((source) -> {
                                    ...
                                    source.play();
                                    net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.client.event.sound.PlaySoundSourceEvent(this, isound, source));
                                 });
                              });
                           } else {
                              this.audioStreamManager.createStreamingResource(sound.getSoundAsOggLocation(), flag2).thenAccept((audioStream) -> {
                                 channelmanager$entry.runOnSoundExecutor((source) -> {
                                    ...
                                    source.play();
                                    net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.client.event.sound.PlayStreamingSourceEvent(this, isound, source));
                                 });
                              });
                           }
