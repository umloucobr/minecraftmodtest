...
public class Framebuffer {
   ...
   public void resize(int width, int height, boolean onMac) {
      if (!RenderSystem.isOnRenderThread()) {
         RenderSystem.recordRenderCall(() -> {
            this.resizeRaw(width, height, onMac);
         });
      } else {
         this.resizeRaw(width, height, onMac);
      }
   ...
   private void resizeRaw(int width, int height, boolean onMac) {
      RenderSystem.assertThread(RenderSystem::isOnRenderThreadOrInit);
   ...
   public void func_237506_a_(Framebuffer buffer) {
      RenderSystem.assertThread(RenderSystem::isOnRenderThreadOrInit);
      if (GlStateManager.isFabulous()) {
         GlStateManager.bindFramebuffer(36008, buffer.framebufferObject);
         GlStateManager.bindFramebuffer(36009, this.framebufferObject);
         GlStateManager.blitFramebuffer(0, 0, buffer.framebufferTextureWidth, buffer.framebufferTextureHeight, 0, 0, this.framebufferTextureWidth, this.framebufferTextureHeight, 256, 9728);
      } else {
         ...
         if (i != 0) {
            ...
            GlStateManager.bindFramebuffer(FramebufferConstants.GL_FRAMEBUFFER, buffer.framebufferObject);
            GlStateManager.copySubImage(3553, 0, 0, 0, 0, 0, Math.min(this.framebufferTextureWidth, buffer.framebufferTextureWidth), Math.min(this.framebufferTextureHeight, buffer.framebufferTextureHeight));
