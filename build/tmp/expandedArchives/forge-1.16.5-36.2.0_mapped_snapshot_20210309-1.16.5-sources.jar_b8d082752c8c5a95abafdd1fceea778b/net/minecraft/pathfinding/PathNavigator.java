...
public abstract class PathNavigator {
   ...
   public void resetSearchDepthMultiplier() {
      this.searchDepthMultiplier = 1.0F;
   }

   public void setSearchDepthMultiplier(float multiplier) {
      this.searchDepthMultiplier = multiplier;
   }
    ...
    */
   @Nullable
   public final Path pathfind(double x, double y, double z, int accuracy) {
      return this.getPathToPos(new BlockPos(x, y, z), accuracy);
   }
    ...
    */
   @Nullable
   public Path pathfind(Stream<BlockPos> targets, int accuracy) {
      return this.pathfind(targets.collect(Collectors.toSet()), 8, false, accuracy);
   }
   ...
   public Path getPathToPos(BlockPos pos, int accuracy) {
      return this.pathfind(ImmutableSet.of(pos), 8, false, accuracy);
   }
    ...
    */
   @Nullable
   protected Path pathfind(Set<BlockPos> targets, int regionOffset, boolean offsetUpward, int accuracy) {
      if (targets.isEmpty()) {
         return null;
         ...
         Path path = this.pathFinder.getNewPath(region, this.entity, targets, f, accuracy, this.searchDepthMultiplier);
   ...
   public boolean tryMoveToXYZ(double x, double y, double z, double speedIn) {
      return this.setPath(this.pathfind(x, y, z, 1), speedIn);
   }
   ...
   public boolean tryMoveToEntityLiving(Entity entityIn, double speedIn) {
      Path path = this.pathfind(entityIn, 1);
   ...
   protected void trimPath() {
      if (this.currentPath != null) {
         for(int i = 0; i < this.currentPath.getCurrentPathLength(); ++i) {
            ...
            if (blockstate.matchesBlock(Blocks.CAULDRON)) {
