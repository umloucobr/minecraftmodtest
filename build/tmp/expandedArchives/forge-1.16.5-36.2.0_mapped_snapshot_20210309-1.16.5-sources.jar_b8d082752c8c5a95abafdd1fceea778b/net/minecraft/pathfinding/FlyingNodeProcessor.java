...
public class FlyingNodeProcessor extends WalkNodeProcessor {
   ...
   public PathNodeType getFloorNodeType(IBlockReader worldIn, int x, int y, int z) {
      ...
      if (pathnodetype == PathNodeType.OPEN && y >= 1) {
         BlockState blockstate = worldIn.getBlockState(blockpos$mutable.setPos(x, y - 1, z));
         PathNodeType pathnodetype1 = func_237238_b_(worldIn, blockpos$mutable.setPos(x, y - 1, z));
         if (pathnodetype1 != PathNodeType.DAMAGE_FIRE && !blockstate.matchesBlock(Blocks.MAGMA_BLOCK) && pathnodetype1 != PathNodeType.LAVA && !blockstate.isIn(BlockTags.CAMPFIRES)) {
   ...
   private PathNodeType getPathNodeType(MobEntity p_192558_1_, int p_192558_2_, int p_192558_3_, int p_192558_4_) {
      return this.determineNodeType(this.blockaccess, p_192558_2_, p_192558_3_, p_192558_4_, p_192558_1_, this.entitySizeX, this.entitySizeY, this.entitySizeZ, this.getCanOpenDoors(), this.getCanEnterDoors());
   }
}