...
public class Path {
   ...
   @OnlyIn(Dist.CLIENT)
   private Set<FlaggedPathPoint> targetPoints;
   private int currentPathIndex;
   private final BlockPos target;
   private final float field_224773_g;
   private final boolean reachesTargetFlag;

   public Path(List<PathPoint> pointsIn, BlockPos targetIn, boolean reachesTargetIn) {
      this.points = pointsIn;
      this.target = targetIn;
      this.field_224773_g = pointsIn.isEmpty() ? Float.MAX_VALUE : this.points.get(this.points.size() - 1).func_224758_c(this.target);
      this.reachesTargetFlag = reachesTargetIn;
   }
   ...
   public void setCurrentPathLength(int length) {
      if (this.points.size() > length) {
         this.points.subList(length, this.points.size()).clear();
      }
