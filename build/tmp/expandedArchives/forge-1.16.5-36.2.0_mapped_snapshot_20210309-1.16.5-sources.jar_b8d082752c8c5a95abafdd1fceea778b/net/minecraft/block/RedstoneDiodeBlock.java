...
public abstract class RedstoneDiodeBlock extends HorizontalBlock {
   ...
   protected int getPowerOnSide(IWorldReader worldIn, BlockPos pos, Direction side) {
      ...
      if (this.isAlternateInput(blockstate)) {
         if (blockstate.matchesBlock(Blocks.REDSTONE_BLOCK)) {
            return 15;
         } else {
            return blockstate.matchesBlock(Blocks.REDSTONE_WIRE) ? blockstate.get(RedstoneWireBlock.POWER) : worldIn.getStrongPower(pos, side);
         }
      } else {
         return 0;
      }
