...
public class BambooBlock extends Block implements IGrowable {
   ...
   public BlockState getStateForPlacement(BlockItemUseContext context) {
      ...
      if (!fluidstate.isEmpty()) {
         ...
         if (blockstate.isIn(BlockTags.BAMBOO_PLANTABLE_ON)) {
            if (blockstate.matchesBlock(Blocks.BAMBOO_SAPLING)) {
               return this.getDefaultState().with(PROPERTY_AGE, Integer.valueOf(0));
            } else if (blockstate.matchesBlock(Blocks.BAMBOO)) {
               ...
               return !blockstate1.matchesBlock(Blocks.BAMBOO) && !blockstate1.matchesBlock(Blocks.BAMBOO_SAPLING) ? Blocks.BAMBOO_SAPLING.getDefaultState() : this.getDefaultState().with(PROPERTY_AGE, blockstate1.get(PROPERTY_AGE));
            }
         } else {
            return null;
         }
   ...
   public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
      ...
      if (facing == Direction.UP && facingState.matchesBlock(Blocks.BAMBOO) && facingState.get(PROPERTY_AGE) > stateIn.get(PROPERTY_AGE)) {
         worldIn.setBlockState(currentPos, stateIn.cycleValue(PROPERTY_AGE), 2);
      }
   ...
   protected void grow(BlockState blockStateIn, World worldIn, BlockPos posIn, Random rand, int maxTotalSize) {
      ...
      if (maxTotalSize >= 1) {
         if (blockstate.matchesBlock(Blocks.BAMBOO) && blockstate.get(PROPERTY_BAMBOO_LEAVES) != BambooLeaves.NONE) {
            if (blockstate.matchesBlock(Blocks.BAMBOO) && blockstate.get(PROPERTY_BAMBOO_LEAVES) != BambooLeaves.NONE) {
               bambooleaves = BambooLeaves.LARGE;
               if (blockstate1.matchesBlock(Blocks.BAMBOO)) {
      ...
      int i = blockStateIn.get(PROPERTY_AGE) != 1 && !blockstate1.matchesBlock(Blocks.BAMBOO) ? 0 : 1;
   ...
   protected int getNumBambooBlocksAbove(IBlockReader worldIn, BlockPos pos) {
      int i;
      for(i = 0; i < 16 && worldIn.getBlockState(pos.up(i + 1)).matchesBlock(Blocks.BAMBOO); ++i) {
      }
   ...
   protected int getNumBambooBlocksBelow(IBlockReader worldIn, BlockPos pos) {
      int i;
      for(i = 0; i < 16 && worldIn.getBlockState(pos.down(i + 1)).matchesBlock(Blocks.BAMBOO); ++i) {
      }
