   ...
   public boolean eventReceived(BlockState state, World worldIn, BlockPos pos, int id, int param) {
      ...
      if (id == 0) {
         ...
         if (this.isSticky) {
            ...
            if (!flag1) {
               if (id != 1 || blockstate1.isAir() || !canPush(blockstate1, worldIn, blockpos, direction.getOpposite(), false, direction) || blockstate1.getPushReaction() != PushReaction.NORMAL && !blockstate1.matchesBlock(Blocks.PISTON) && !blockstate1.matchesBlock(Blocks.STICKY_PISTON)) {
   ...
   public static boolean canPush(BlockState blockStateIn, World worldIn, BlockPos pos, Direction facing, boolean destroyBlocks, Direction direction) {
      if (pos.getY() >= 0 && pos.getY() <= worldIn.getHeight() - 1 && worldIn.getWorldBorder().contains(pos)) {
         if (blockStateIn.isAir()) {
            return true;
         } else if (!blockStateIn.matchesBlock(Blocks.OBSIDIAN) && !blockStateIn.matchesBlock(Blocks.CRYING_OBSIDIAN) && !blockStateIn.matchesBlock(Blocks.RESPAWN_ANCHOR)) {
            if (facing == Direction.DOWN && pos.getY() == 0) {
               return false;
            } else if (facing == Direction.UP && pos.getY() == worldIn.getHeight() - 1) {
               return false;
            } else {
               if (!blockStateIn.matchesBlock(Blocks.PISTON) && !blockStateIn.matchesBlock(Blocks.STICKY_PISTON)) {
