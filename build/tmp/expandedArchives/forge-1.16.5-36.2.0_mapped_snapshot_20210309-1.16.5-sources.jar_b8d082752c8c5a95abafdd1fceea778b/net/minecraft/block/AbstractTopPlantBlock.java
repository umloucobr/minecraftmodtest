...
public abstract class AbstractTopPlantBlock extends AbstractPlantBlock implements IGrowable {
   ...
   public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
      if (state.get(AGE) < 25 && net.minecraftforge.common.ForgeHooks.onCropsGrowPre(worldIn, pos.offset(this.growthDirection), worldIn.getBlockState(pos.offset(this.growthDirection)),random.nextDouble() < this.growthChance)) {
         ...
         if (this.canGrowIn(worldIn.getBlockState(blockpos))) {
            worldIn.setBlockState(blockpos, state.cycleValue(AGE));
   ...
   public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
      ...
      if (facing != this.growthDirection || !facingState.matchesBlock(this) && !facingState.matchesBlock(this.getBodyPlantBlock())) {
