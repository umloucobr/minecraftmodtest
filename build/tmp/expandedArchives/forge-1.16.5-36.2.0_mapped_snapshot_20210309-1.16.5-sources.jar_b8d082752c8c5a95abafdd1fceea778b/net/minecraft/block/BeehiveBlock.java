...
public class BeehiveBlock extends ContainerBlock {
   ...
   @Override public BlockState rotate(BlockState blockState, net.minecraft.util.Rotation rotation) { return (BlockState)blockState.with(FACING, rotation.rotate(blockState.get(FACING))); }
   @Override public BlockState mirror(BlockState blockState, net.minecraft.util.Mirror mirror) { return blockState.rotate(mirror.toRotation(blockState.get(FACING))); }
   ...
   private void addHoneyParticle(World world, BlockPos pos, BlockState state) {
      if (state.getFluidState().isEmpty() && !(world.rand.nextFloat() < 0.3F)) {
         VoxelShape voxelshape = state.getCollisionShapeUncached(world, pos);
         ...
         if (d0 >= 1.0D && !state.isIn(BlockTags.IMPERMEABLE)) {
            ...
            if (d1 > 0.0D) {
               ...
               VoxelShape voxelshape1 = blockstate.getCollisionShapeUncached(world, blockpos);
