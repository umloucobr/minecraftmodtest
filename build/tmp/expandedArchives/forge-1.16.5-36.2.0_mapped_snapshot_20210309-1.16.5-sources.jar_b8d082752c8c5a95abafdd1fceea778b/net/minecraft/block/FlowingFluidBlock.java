...
public class FlowingFluidBlock extends Block implements IBucketPickupHandler {
   ...
   protected synchronized void initFluidStateCache() {
      if (fluidStateCacheInitialized == false) {
         this.fluidStatesCache.add(getFluid().getStillFluidState(false));

         for (int i = 1; i < 8; ++i)
            this.fluidStatesCache.add(getFluid().getFlowingFluidState(8 - i, false));

         this.fluidStatesCache.add(getFluid().getFlowingFluidState(8, true));
         fluidStateCacheInitialized = true;
      }
   }
}
