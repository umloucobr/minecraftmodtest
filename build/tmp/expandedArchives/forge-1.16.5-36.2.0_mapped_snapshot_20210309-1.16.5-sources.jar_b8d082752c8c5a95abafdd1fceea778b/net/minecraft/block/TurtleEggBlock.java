...
public class TurtleEggBlock extends Block {
   ...
   private void tryTrample(World worldIn, BlockPos pos, Entity trampler, int chances) {
      if (this.canTrample(worldIn, trampler)) {
         if (!worldIn.isRemote && worldIn.rand.nextInt(chances) == 0) {
            ...
            if (blockstate.matchesBlock(Blocks.TURTLE_EGG)) {
   ...
   public BlockState getStateForPlacement(BlockItemUseContext context) {
      ...
      return blockstate.matchesBlock(this) ? blockstate.with(EGGS, Integer.valueOf(Math.min(4, blockstate.get(EGGS) + 1))) : super.getStateForPlacement(context);
   }
