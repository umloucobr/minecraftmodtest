...
public abstract class AbstractBlock extends net.minecraftforge.registries.ForgeRegistryEntry<Block> {
   ...
   public abstract static class AbstractBlockState extends StateHolder<Block, BlockState> {
      ...
      public VoxelShape getRenderShape(IBlockReader reader, BlockPos pos) {
      ...
      public boolean isOpaqueCube(IBlockReader worldIn, BlockPos pos) {
         if (this.cache != null) {
            ...
            return blockstate.isSolid() ? Block.isOpaque(blockstate.getRenderShape(worldIn, pos)) : false;
         }
      }
      ...
      @OnlyIn(Dist.CLIENT)
      public boolean shouldBlockVision(IBlockReader worldIn, BlockPos pos) {
      ...
      public boolean matchesBlock(Block tagIn) {
      ...
      public boolean hasOpaqueCollisionShape(IBlockReader reader, BlockPos pos) {
         return this.cache != null ? this.cache.opaqueCollisionShape : Block.isOpaque(this.getCollisionShapeUncached(reader, pos));
      }
         ...
         private Cache(BlockState stateIn) {
            ...
            for(Direction direction1 : DIRECTIONS) {
               for(BlockVoxelShape blockvoxelshape : BlockVoxelShape.values()) {
                  this.solidSides[packDirectionAndShape(direction1, blockvoxelshape)] = blockvoxelshape.func_241854_a(stateIn, EmptyBlockReader.INSTANCE, BlockPos.ZERO, direction1);
               }
            }

            this.opaqueCollisionShape = Block.isOpaque(stateIn.getCollisionShapeUncached(EmptyBlockReader.INSTANCE, BlockPos.ZERO));
         }

         public boolean isSolidSide(Direction direction, BlockVoxelShape blockVoxelShape) {
            return this.solidSides[packDirectionAndShape(direction, blockVoxelShape)];
         }

         private static int packDirectionAndShape(Direction direction, BlockVoxelShape blockVoxelShape) {
