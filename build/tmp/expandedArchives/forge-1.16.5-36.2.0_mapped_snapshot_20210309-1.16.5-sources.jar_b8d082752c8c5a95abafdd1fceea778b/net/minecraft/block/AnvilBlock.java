...
public class AnvilBlock extends FallingBlock {
   ...
   public static BlockState damage(BlockState state) {
      if (state.matchesBlock(Blocks.ANVIL)) {
         return Blocks.CHIPPED_ANVIL.getDefaultState().with(FACING, state.get(FACING));
      } else {
         return state.matchesBlock(Blocks.CHIPPED_ANVIL) ? Blocks.DAMAGED_ANVIL.getDefaultState().with(FACING, state.get(FACING)) : null;
      }
   }
