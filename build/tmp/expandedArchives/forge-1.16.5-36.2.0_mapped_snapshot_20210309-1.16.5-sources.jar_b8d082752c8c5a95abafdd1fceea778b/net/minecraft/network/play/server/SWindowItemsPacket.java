...
public class SWindowItemsPacket implements IPacket<IClientPlayNetHandler> {
   ...
   public SWindowItemsPacket(int windowId, NonNullList<ItemStack> stacks) {
      this.windowId = windowId;
      this.itemStacks = NonNullList.withSize(stacks.size(), ItemStack.EMPTY);

      for(int i = 0; i < this.itemStacks.size(); ++i) {
         this.itemStacks.set(i, stacks.get(i).copy());
      }
