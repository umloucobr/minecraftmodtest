...
public class SEntityVelocityPacket implements IPacket<IClientPlayNetHandler> {
   ...
   public SEntityVelocityPacket(int entityId, Vector3d motionVector) {
      this.entityID = entityId;
      double d0 = 3.9D;
      double d1 = MathHelper.clamp(motionVector.x, -3.9D, 3.9D);
      double d2 = MathHelper.clamp(motionVector.y, -3.9D, 3.9D);
      double d3 = MathHelper.clamp(motionVector.z, -3.9D, 3.9D);
