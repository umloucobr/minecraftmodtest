...
public class AddBedTileEntity extends DataFix {
   ...
   private <TE> TypeRewriteRule cap(Type<?> levelType, ListType<TE> tileType) {
      Type<TE> type = tileType.getElement();
      OpticFinder<?> opticfinder = DSL.fieldFinder("Level", levelType);
      OpticFinder<List<TE>> opticfinder1 = DSL.fieldFinder("TileEntities", tileType);
      int i = 416;
      return TypeRewriteRule.seq(this.fixTypeEverywhere("InjectBedBlockEntityType", this.getInputSchema().findChoiceType(TypeReferences.BLOCK_ENTITY), this.getOutputSchema().findChoiceType(TypeReferences.BLOCK_ENTITY), (p_233085_0_) -> {
         ...
         for(int l = 0; l < list1.size(); ++l) {
            ...
            Stream<Integer> stream = dynamic1.get("Blocks").asStream().map((dynamicBlock) -> {
               return dynamicBlock.asInt(0);
            });
            int j1 = 0;
