...
public class SortedArraySet<T> extends AbstractSet<T> {
   ...
   private SortedArraySet(int initialCapacity, Comparator<T> comparator) {
      ...
      if (initialCapacity < 0) {
         ...
         this.storage = (T[])cast(new Object[initialCapacity]);
      }
   }

   public static <T extends Comparable<T>> SortedArraySet<T> newSet(int initialCapacity) {
   ...
   private int binarySearch(T object) {
      return Arrays.binarySearch(this.storage, 0, this.maxIndex, object, this.comparator);
   }
   ...
   class Itr implements Iterator<T> {
      ...
      public boolean hasNext() {
         return this.currentIndex < SortedArraySet.this.maxIndex;
      }

      public T next() {
         if (this.currentIndex >= SortedArraySet.this.maxIndex) {
            throw new NoSuchElementException();
         } else {
            this.previousIndex = this.currentIndex++;
            return SortedArraySet.this.storage[this.previousIndex];
         }
      }
      ...
      public void remove() {
         if (this.previousIndex == -1) {
            ...
            SortedArraySet.this.func_226183_d_(this.previousIndex);
            --this.currentIndex;
