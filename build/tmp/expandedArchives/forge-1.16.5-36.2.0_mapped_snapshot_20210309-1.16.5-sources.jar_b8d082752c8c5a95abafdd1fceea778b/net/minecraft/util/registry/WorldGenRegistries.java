...
public class WorldGenRegistries {
   ...
   public static final Registry<DimensionSettings> NOISE_SETTINGS = createRegistry(Registry.NOISE_SETTINGS_KEY, DimensionSettings::getDefaultDimensionSettings);
   ...
   private static <T extends net.minecraftforge.registries.IForgeRegistryEntry<T>> Registry<T> forge(RegistryKey<? extends Registry<T>> key, Supplier<T> def) {
      return createRegistry(key, net.minecraftforge.registries.GameData.getWrapper(key, Lifecycle.stable()), def, Lifecycle.stable());
   }
