...
public abstract class Registry<T> implements Codec<T>, Keyable, IObjectIntIterable<T> {
   ...
   @Deprecated public static final Registry<Effect> EFFECTS = forge(MOB_EFFECT_KEY, () -> {
      return Effects.LUCK;
   });
   ...
   @Deprecated public static final Registry<TileEntityType<?>> BLOCK_ENTITY_TYPE = forge(BLOCK_ENTITY_TYPE_KEY, () -> {
      return TileEntityType.FURNACE;
   });
   @Deprecated public static final DefaultedRegistry<PaintingType> MOTIVE = forge(MOTIVE_KEY, "kebab", () -> {
   ...
   @Deprecated public static final Registry<ContainerType<?>> MENU = forge(MENU_KEY, () -> {
      return ContainerType.ANVIL;
   });
   public static final Registry<IRecipeType<?>> RECIPE_TYPE = createRegistry(RECIPE_TYPE_KEY, () -> {
      return IRecipeType.CRAFTING;
   });
   @Deprecated public static final Registry<IRecipeSerializer<?>> RECIPE_SERIALIZER = forge(RECIPE_SERIALIZER_KEY, () -> {
      return IRecipeSerializer.CRAFTING_SHAPELESS;
   });
   @Deprecated public static final Registry<Attribute> ATTRIBUTE = forge(ATTRIBUTE_KEY, () -> {
      return Attributes.LUCK;
   });
   @Deprecated public static final Registry<StatType<?>> STATS = forge(STAT_TYPE_KEY, () -> {
   ...
   @Deprecated public static final Registry<SurfaceBuilder<?>> SURFACE_BUILDER = forge(SURFACE_BUILDER_KEY, () -> {
      return SurfaceBuilder.DEFAULT;
   });
   public static final RegistryKey<Registry<WorldCarver<?>>> CARVER_KEY = createKey("worldgen/carver");
   @Deprecated public static final Registry<WorldCarver<?>> CARVER = forge(CARVER_KEY, () -> {
   ...
   @Deprecated public static final Registry<Structure<?>> STRUCTURE_FEATURE = forge(STRUCTURE_FEATURE_KEY, () -> {
   ...
   public static final Registry<IStructurePieceType> STRUCTURE_PIECE = createRegistry(STRUCTURE_PIECE_KEY, () -> {
      return IStructurePieceType.MINESHAFT_ROOM;
   });
   public static final RegistryKey<Registry<Placement<?>>> DECORATOR_KEY = createKey("worldgen/decorator");
   @Deprecated public static final Registry<Placement<?>> DECORATOR = forge(DECORATOR_KEY, () -> {
   ...
   private static <T extends net.minecraftforge.registries.IForgeRegistryEntry<T>> Registry<T> forge(RegistryKey<? extends Registry<T>> key, Lifecycle cycle, Supplier<T> def) {
      return addRegistry(key, net.minecraftforge.registries.GameData.getWrapper(key, cycle), def, cycle);
   }
   ...
   private static <T extends net.minecraftforge.registries.IForgeRegistryEntry<T>> DefaultedRegistry<T> forge(RegistryKey<? extends Registry<T>> key, String defKey, Lifecycle cycle, Supplier<T> def) {
      return addRegistry(key, net.minecraftforge.registries.GameData.getWrapper(key, cycle, defKey), def, cycle);
   }
   ...
   private static <T, R extends MutableRegistry<T>> R addRegistry(RegistryKey<? extends Registry<T>> registryKey, R instance, Supplier<T> objectSupplier, Lifecycle lifecycle) {
      ...
      REGISTRY_NAME_TO_DEFAULT.put(resourcelocation, objectSupplier);
   ...
   static {
      ...
      REGISTRY_NAME_TO_DEFAULT.forEach((registry, registrySupplier) -> {
