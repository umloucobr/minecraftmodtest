...
public class WorldOptimizer {
   ...
   private final ImmutableSet<RegistryKey<World>> dimensionKeys;
   private final boolean clearLightAndHeightMaps;
   ...
   public WorldOptimizer(SaveFormat.LevelSave worldStorage, DataFixer fixer, ImmutableSet<RegistryKey<World>> dimensionKeys, boolean clearLightAndHeightMaps) {
      this.dimensionKeys = dimensionKeys;
      this.clearLightAndHeightMaps = clearLightAndHeightMaps;
      this.fixer = fixer;
      this.worldStorage = worldStorage;
      this.savedDataManager = new DimensionSavedDataManager(new File(this.worldStorage.getDimensionFolder(World.OVERWORLD), "data"), fixer);
   ...
   private void optimize() {
      ...
      if (this.totalChunks == 0) {
         ...
         for(RegistryKey<World> registrykey1 : this.dimensionKeys) {
            File file1 = this.worldStorage.getDimensionFolder(registrykey1);
            builder1.put(registrykey1, new ChunkLoader(new File(file1, "region"), this.fixer, true));
         }
         ...
         while(this.active) {
            ...
            for(RegistryKey<World> registrykey2 : this.dimensionKeys) {
               ...
               if (listiterator.hasNext()) {
                  ...
                  try {
                     ...
                     if (compoundnbt != null) {
                        ...
                        if (this.clearLightAndHeightMaps) {
