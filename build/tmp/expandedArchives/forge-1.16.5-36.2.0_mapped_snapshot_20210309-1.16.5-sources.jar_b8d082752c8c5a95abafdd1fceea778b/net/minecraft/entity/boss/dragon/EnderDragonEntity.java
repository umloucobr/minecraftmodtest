...
public class EnderDragonEntity extends MobEntity implements IMob {
   ...
   public EnderDragonEntity(EntityType<? extends EnderDragonEntity> type, World worldIn) {
      ...
      if (worldIn instanceof ServerWorld) {
         this.fightManager = ((ServerWorld)worldIn).getDragonFightManager();
      } else {
   ...
   public Path findPath(int startIdx, int finishIdx, @Nullable PathPoint andThen) {
      for(int i = 0; i < 24; ++i) {
         ...
         pathpoint.totalCost = 0.0F;
         pathpoint.accumulatedCost = 0.0F;
         pathpoint.costToTarget = 0.0F;
      ...
      pathpoint4.accumulatedCost = 0.0F;
      pathpoint4.costToTarget = pathpoint4.distanceTo(pathpoint5);
      pathpoint4.totalCost = pathpoint4.costToTarget;
         ...
         for(int i1 = j; i1 < 24; ++i1) {
            if ((this.neighbors[k] & 1 << i1) > 0) {
               ...
               if (!pathpoint3.visited) {
                  float f = pathpoint2.accumulatedCost + pathpoint2.distanceTo(pathpoint3);
                  if (!pathpoint3.isAssigned() || f < pathpoint3.accumulatedCost) {
                     pathpoint3.previous = pathpoint2;
                     pathpoint3.accumulatedCost = f;
                     pathpoint3.costToTarget = pathpoint3.distanceTo(pathpoint5);
                     if (pathpoint3.isAssigned()) {
                        this.pathFindQueue.changeTotalCost(pathpoint3, pathpoint3.accumulatedCost + pathpoint3.costToTarget);
                     } else {
                        pathpoint3.totalCost = pathpoint3.accumulatedCost + pathpoint3.costToTarget;
