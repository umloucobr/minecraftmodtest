...
public class VillagerEntity extends AbstractVillagerEntity implements IReputationTracking, IVillagerDataHolder {
   ...
   private void initBrain(Brain<VillagerEntity> villagerBrain) {
      ...
      villagerBrain.setPersistentActivities(ImmutableSet.of(Activity.CORE));
   ...
   public VillagerEntity createChild(ServerWorld world, AgeableEntity mate) {
      double d0 = this.rand.nextDouble();
      VillagerType villagertype;
      if (d0 < 0.5D) {
         villagertype = VillagerType.func_242371_a(world.func_242406_i(this.getPosition()));
      } else if (d0 < 0.75D) {
         villagertype = this.getVillagerData().getType();
      } else {
         villagertype = ((VillagerEntity)mate).getVillagerData().getType();
      }
   ...
   public void causeLightningStrike(ServerWorld world, LightningBoltEntity lightning) {
      if (world.getDifficulty() != Difficulty.PEACEFUL && net.minecraftforge.event.ForgeEventFactory.canLivingConvert(this, EntityType.WITCH, (timer) -> {})) {
         ...
         witchentity.onInitialSpawn(world, world.getDifficultyForLocation(witchentity.getPosition()), SpawnReason.CONVERSION, (ILivingEntityData)null, (CompoundNBT)null);
         ...
         world.func_242417_l(witchentity);
         this.func_242369_fq();
         this.remove();
      } else {
