...
public class PandaEntity extends AnimalEntity {
   ...
   public AgeableEntity createChild(ServerWorld world, AgeableEntity mate) {
      ...
      if (mate instanceof PandaEntity) {
         pandaentity.getGenesForChildFromParents(this, (PandaEntity)mate);
      }
   ...
   public ActionResultType getEntityInteractionResult(PlayerEntity playerIn, Hand hand) {
      ...
      if (this.func_213566_eo()) {
         ...
         if (this.isChild()) {
            ...
            if (!itemstack1.isEmpty() && !playerIn.abilities.isCreativeMode) {
            ...
            for(int j = 0; j < 8; ++j) {
               for(int k = 0; k <= j; k = k > 0 ? -k : 1 - k) {
                  for(int l = k < j && k > -j ? j : 0; l <= j; l = l > 0 ? -l : 1 - l) {
                     ...
                     if (this.world.getBlockState(blockpos$mutable).matchesBlock(Blocks.BAMBOO)) {
                        return true;
                     }
                  }
               }
