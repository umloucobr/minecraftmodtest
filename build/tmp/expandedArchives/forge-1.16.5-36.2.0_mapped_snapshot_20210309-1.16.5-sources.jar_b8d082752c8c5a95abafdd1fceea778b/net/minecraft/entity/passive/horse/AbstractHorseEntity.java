...
public abstract class AbstractHorseEntity extends AnimalEntity implements IInventoryChangedListener, IJumpingMount, IEquipable {
   ...
   private static final EntityPredicate MOMMY_TARGETING = (new EntityPredicate()).setDistance(16.0D).allowInvulnerable().allowFriendlyFire().setIgnoresLineOfSight().setCustomPredicate(IS_HORSE_BREEDING);
   ...
   public void livingTick() {
      ...
      if (!this.world.isRemote && this.isAlive()) {
         ...
         if (this.canEatGrass()) {
            if (!this.isEatingHaystack() && !this.isBeingRidden() && this.rand.nextInt(300) == 0 && this.world.getBlockState(this.getPosition().down()).matchesBlock(Blocks.GRASS_BLOCK)) {
   ...
   public Vector3d getDismountPosition(LivingEntity livingEntity) {
      Vector3d vector3d = getRiderDismountPlacementOffset((double)this.getWidth(), (double)livingEntity.getWidth(), this.rotationYaw + (livingEntity.getPrimaryHand() == HandSide.RIGHT ? 90.0F : -90.0F));
      Vector3d vector3d1 = this.func_234236_a_(vector3d, livingEntity);
      if (vector3d1 != null) {
         return vector3d1;
      } else {
         Vector3d vector3d2 = getRiderDismountPlacementOffset((double)this.getWidth(), (double)livingEntity.getWidth(), this.rotationYaw + (livingEntity.getPrimaryHand() == HandSide.LEFT ? 90.0F : -90.0F));
