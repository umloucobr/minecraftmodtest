...
public class FoxEntity extends AnimalEntity {
   ...
   public FoxEntity createChild(ServerWorld world, AgeableEntity mate) {
      FoxEntity foxentity = EntityType.FOX.create(world);
      foxentity.setVariantType(this.rand.nextBoolean() ? this.getVariantType() : ((FoxEntity)mate).getVariantType());
      return foxentity;
   }
   ...
   abstract class BaseGoal extends Goal {
      private final EntityPredicate field_220816_b = (new EntityPredicate()).setDistance(12.0D).setIgnoresLineOfSight().setCustomPredicate(FoxEntity.this.new AlertablePredicate());
   ...
   public class EatBerriesGoal extends MoveToBlockGoal {
      ...
      protected boolean shouldMoveTo(IWorldReader worldIn, BlockPos pos) {
         ...
         return blockstate.matchesBlock(Blocks.SWEET_BERRY_BUSH) && blockstate.get(SweetBerryBushBlock.AGE) >= 2;
      }
   ...
   public class PounceGoal extends net.minecraft.entity.ai.goal.JumpGoal {
         ...
         if (!FoxEntity.this.func_213490_ee()) {
            ...
            if (livingentity != null && livingentity.isAlive()) {
               if (livingentity.getAdjustedHorizontalFacing() != livingentity.getHorizontalFacing()) {
                  ...
                  if (!flag) {
                     FoxEntity.this.getNavigator().pathfind(livingentity, 0);
                     FoxEntity.this.setCrouching(false);
                     FoxEntity.this.func_213502_u(false);
                  }
      ...
      public void tick() {
         ...
         } else if (FoxEntity.this.rotationPitch > 0.0F && FoxEntity.this.onGround && (float)FoxEntity.this.getMotion().y != 0.0F && FoxEntity.this.world.getBlockState(FoxEntity.this.getPosition()).matchesBlock(Blocks.SNOW)) {
