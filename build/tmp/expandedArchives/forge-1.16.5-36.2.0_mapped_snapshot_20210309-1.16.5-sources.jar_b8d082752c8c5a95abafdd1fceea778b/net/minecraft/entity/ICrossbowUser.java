...
public interface ICrossbowUser extends IRangedAttackMob {
   ...
   void fireProjectile(LivingEntity p_230284_1_, ItemStack p_230284_2_, ProjectileEntity p_230284_3_, float p_230284_4_);
   ...
   default void func_234281_b_(LivingEntity shooter, float velocity) {
      Hand hand = ProjectileHelper.getWeaponHoldingHand(shooter, item -> item instanceof CrossbowItem);
      ItemStack itemstack = shooter.getHeldItem(hand);
      if (shooter.func_233634_a_(item -> item instanceof CrossbowItem)) {
         CrossbowItem.fireProjectiles(shooter.world, shooter, hand, itemstack, velocity, (float)(14 - shooter.world.getDifficulty().getId() * 4));
      }
