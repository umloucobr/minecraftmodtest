...
public abstract class PlayerEntity extends LivingEntity {
      ...
      if (this.isPassenger()) {
         ...
         if (i > 0) {
            ...
            if (entity instanceof AbstractMinecartEntity) {
               ...
               this.addStat(Stats.STRIDER_ONE_CM, i);
            }
         }
      }
   ...
   public void onKillEntity(ServerWorld world, LivingEntity killedEntity) {
      this.addStat(Stats.ENTITY_KILLED.get(killedEntity.getType()));
   }
   ...
   public ITextComponent getDisplayName() {
      ...
      iformattabletextcomponent = prefixes.stream().reduce(iformattabletextcomponent, IFormattableTextComponent::appendSibling);
      iformattabletextcomponent = iformattabletextcomponent.appendSibling(ScorePlayerTeam.func_237500_a_(this.getTeam(), this.displayname));
   ...
   public Vector3d getLeashPosition(float partialTicks) {
      ...
      if (!this.isElytraFlying() && !this.isSpinAttacking()) {
         if (this.isActualySwimming()) {
            return this.getLeashHandPosition(partialTicks).add((new Vector3d(d0, 0.2D, -0.15D)).rotatePitch(-f).rotateYaw(-f1));
         } else {
            double d5 = this.getBoundingBox().getYSize() - 1.0D;
            double d6 = this.isCrouching() ? -0.2D : 0.07D;
            return this.getLeashHandPosition(partialTicks).add((new Vector3d(d0, d5, d6)).rotateYaw(-f1));
         }
      } else {
         ...
         return this.getLeashHandPosition(partialTicks).add((new Vector3d(d0, -0.11D, 0.85D)).rotateRoll(-f2).rotatePitch(-f).rotateYaw(-f1));
      }
   }
