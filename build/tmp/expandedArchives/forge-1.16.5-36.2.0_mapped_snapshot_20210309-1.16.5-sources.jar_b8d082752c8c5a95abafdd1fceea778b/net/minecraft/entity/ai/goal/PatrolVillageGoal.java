...
public class PatrolVillageGoal extends RandomWalkingGoal {
   ...
   private Vector3d func_234033_l_() {
      ...
      if (sectionpos == null) {
         ...
         BlockPos blockpos = this.getVillageCenterPos(sectionpos);
   ...
   private SectionPos func_234034_m_() {
      ServerWorld serverworld = (ServerWorld)this.creature.world;
      List<SectionPos> list = SectionPos.getAllInBox(SectionPos.from(this.creature), 2).filter((sectionPos) -> {
   ...
   @Nullable
   private BlockPos getVillageCenterPos(SectionPos sectionPos) {
      ServerWorld serverworld = (ServerWorld)this.creature.world;
      PointOfInterestManager pointofinterestmanager = serverworld.getPointOfInterestManager();
      List<BlockPos> list = pointofinterestmanager.func_219146_b((poiType) -> {
         return true;
      }, sectionPos.getCenter(), 8, PointOfInterestManager.Status.IS_OCCUPIED).map(PointOfInterest::getPos).collect(Collectors.toList());
      return list.isEmpty() ? null : list.get(serverworld.rand.nextInt(list.size()));
   }
