...
public class BreakBlockGoal extends MoveToBlockGoal {
   ...
   private BlockPos findTarget(BlockPos pos, IBlockReader worldIn) {
      if (worldIn.getBlockState(pos).matchesBlock(this.block)) {
         return pos;
      } else {
         BlockPos[] ablockpos = new BlockPos[]{pos.down(), pos.west(), pos.east(), pos.north(), pos.south(), pos.down().down()};

         for(BlockPos blockpos : ablockpos) {
            if (worldIn.getBlockState(blockpos).matchesBlock(this.block)) {
               return blockpos;
            }
         }
   ...
   protected boolean shouldMoveTo(IWorldReader worldIn, BlockPos pos) {
      ...
      if (ichunk == null) {
         ...
         return ichunk.getBlockState(pos).matchesBlock(this.block) && ichunk.getBlockState(pos.up()).isAir() && ichunk.getBlockState(pos.up(2)).isAir();
      }
   }
}
