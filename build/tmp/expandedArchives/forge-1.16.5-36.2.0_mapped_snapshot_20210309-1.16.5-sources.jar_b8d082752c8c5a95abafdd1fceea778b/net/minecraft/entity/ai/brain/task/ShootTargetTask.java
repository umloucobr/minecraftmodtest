...
public class ShootTargetTask<E extends MobEntity & ICrossbowUser, T extends LivingEntity> extends Task<E> {
   ...
   protected boolean shouldExecute(ServerWorld worldIn, E owner) {
      ...
      return owner.func_233634_a_(item -> item instanceof CrossbowItem) && BrainUtil.isMobVisible(owner, livingentity) && BrainUtil.canFireAtTarget(owner, livingentity, 0);
   }
   ...
   protected void resetTask(ServerWorld worldIn, E entityIn, long gameTimeIn) {
      ...
      if (entityIn.func_233634_a_(item -> item instanceof CrossbowItem)) {
   ...
   private void func_233888_a_(E p_233888_1_, LivingEntity p_233888_2_) {
      if (this.field_233886_c_ == ShootTargetTask.Status.UNCHARGED) {
         p_233888_1_.setActiveHand(ProjectileHelper.getWeaponHoldingHand(p_233888_1_, item -> item instanceof CrossbowItem));
         ...
         ItemStack itemstack1 = p_233888_1_.getHeldItem(ProjectileHelper.getWeaponHoldingHand(p_233888_1_, item -> item instanceof CrossbowItem));
