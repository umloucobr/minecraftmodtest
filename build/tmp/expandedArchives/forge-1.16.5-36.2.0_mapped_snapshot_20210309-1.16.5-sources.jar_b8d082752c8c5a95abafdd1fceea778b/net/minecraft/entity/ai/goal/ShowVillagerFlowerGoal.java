...
public class ShowVillagerFlowerGoal extends Goal {
   private static final EntityPredicate VILLAGER_PREDICATE = (new EntityPredicate()).setDistance(6.0D).allowFriendlyFire().allowInvulnerable();
   ...
   public boolean shouldExecute() {
      if (!this.ironGolem.world.isDaytime()) {
         ...
         this.villager = this.ironGolem.world.getClosestEntityWithinAABB(VillagerEntity.class, VILLAGER_PREDICATE, this.ironGolem, this.ironGolem.getPosX(), this.ironGolem.getPosY(), this.ironGolem.getPosZ(), this.ironGolem.getBoundingBox().grow(6.0D, 2.0D, 6.0D));
