...
public class EndGatewayTileEntity extends EndPortalTileEntity implements ITickableTileEntity {
   ...
   public static boolean func_242690_a(Entity p_242690_0_) {
      return EntityPredicates.NOT_SPECTATING.test(p_242690_0_) && !p_242690_0_.getLowestRidingEntity().hasPortalCooldown();
   }
      ...
      if (this.world instanceof ServerWorld && !this.isCoolingDown()) {
         ...
         if (this.exitPortal != null) {
            ...
            if (entityIn instanceof EnderPearlEntity) {
               Entity entity1 = ((EnderPearlEntity)entityIn).getShooter();
         ...
         for(int j = -radius; j <= radius; ++j) {
            if (i != 0 || j != 0 || allowBedrock) {
               for(int k = 255; k > (blockpos == null ? 0 : blockpos.getY()); --k) {
                  ...
                  if (blockstate.hasOpaqueCollisionShape(worldIn, blockpos1) && (allowBedrock || !blockstate.matchesBlock(Blocks.BEDROCK))) {
                     blockpos = blockpos1;
                     break;
                  }
               }
   ...
   private static BlockPos findSpawnpointInChunk(Chunk chunkIn) {
      ...
      for(BlockPos blockpos3 : BlockPos.getAllInBoxMutable(blockpos, blockpos1)) {
         ...
         if (blockstate.matchesBlock(Blocks.END_STONE) && !chunkIn.getBlockState(blockpos4).hasOpaqueCollisionShape(chunkIn, blockpos4) && !chunkIn.getBlockState(blockpos5).hasOpaqueCollisionShape(chunkIn, blockpos5)) {
   ...
   private void func_227016_a_(ServerWorld p_227016_1_, BlockPos p_227016_2_) {
      Feature.END_GATEWAY.withConfiguration(EndGatewayConfig.create(this.getPos(), false)).generate(p_227016_1_, p_227016_1_.getChunkProvider().getChunkGenerator(), new Random(), p_227016_2_);
   }
