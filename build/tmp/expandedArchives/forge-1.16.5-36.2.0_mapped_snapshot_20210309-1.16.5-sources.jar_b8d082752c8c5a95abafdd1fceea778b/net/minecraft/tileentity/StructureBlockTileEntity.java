...
public class StructureBlockTileEntity extends TileEntity {
   ...
   public void setName(@Nullable ResourceLocation newName) {
      this.name = newName;
   }
   ...
   private List<StructureBlockTileEntity> filterRelatedCornerBlocks(List<StructureBlockTileEntity> structureBlocks) {
      Predicate<StructureBlockTileEntity> predicate = (structureBlock) -> {
         return structureBlock.mode == StructureMode.CORNER && Objects.equals(this.name, structureBlock.name);
      };
   ...
   public boolean func_242689_a(ServerWorld world, boolean p_242689_2_, Template p_242689_3_) {
      BlockPos blockpos = this.getPos();
      ...
      if (p_242689_2_ && !flag) {
         ...
         if (this.integrity < 1.0F) {
            placementsettings.clearProcessors().addProcessor(new IntegrityProcessor(MathHelper.clamp(this.integrity, 0.0F, 1.0F))).setRandom(getRandom(this.seed));
         }

         BlockPos blockpos2 = blockpos.add(this.position);
         p_242689_3_.func_237144_a_(world, blockpos2, placementsettings, getRandom(this.seed));
         return true;
      }
   }
