...
public class ShearsItem extends Item {
   ...
   public boolean onBlockDestroyed(ItemStack stack, World worldIn, BlockState state, BlockPos pos, LivingEntity entityLiving) {
      ...
      return !state.isIn(BlockTags.LEAVES) && !state.matchesBlock(Blocks.COBWEB) && !state.matchesBlock(Blocks.GRASS) && !state.matchesBlock(Blocks.FERN) && !state.matchesBlock(Blocks.DEAD_BUSH) && !state.matchesBlock(Blocks.VINE) && !state.matchesBlock(Blocks.TRIPWIRE) && !state.isIn(BlockTags.WOOL) ? super.onBlockDestroyed(stack, worldIn, state, pos, entityLiving) : true;
   }
   ...
   public boolean canHarvestBlock(BlockState blockIn) {
      return blockIn.matchesBlock(Blocks.COBWEB) || blockIn.matchesBlock(Blocks.REDSTONE_WIRE) || blockIn.matchesBlock(Blocks.TRIPWIRE);
   }
