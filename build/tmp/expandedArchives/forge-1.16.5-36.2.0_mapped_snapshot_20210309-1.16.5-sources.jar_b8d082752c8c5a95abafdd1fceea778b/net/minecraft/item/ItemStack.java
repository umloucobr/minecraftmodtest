...
public final class ItemStack extends net.minecraftforge.common.capabilities.CapabilityProvider<ItemStack> implements net.minecraftforge.common.extensions.IForgeItemStack {
         ...
         for(EquipmentSlotType equipmentslottype : EquipmentSlotType.values()) {
            ...
            if (!multimap.isEmpty()) {
               ...
               for(Entry<Attribute, AttributeModifier> entry : multimap.entries()) {
                  ...
                  if (flag) {
                     list.add((new StringTextComponent(" ")).appendSibling(new TranslationTextComponent("attribute.modifier.equals." + attributemodifier.getOperation().getId(), DECIMALFORMAT.format(d1), new TranslationTextComponent(entry.getKey().getAttributeName()))).mergeStyle(TextFormatting.DARK_GREEN));
                  } else if (d0 > 0.0D) {
   ...
   public ITextComponent getTextComponent() {
      IFormattableTextComponent iformattabletextcomponent = (new StringTextComponent("")).appendSibling(this.getDisplayName());
      if (this.hasDisplayName()) {
         iformattabletextcomponent.mergeStyle(TextFormatting.ITALIC);
      }
