...
public class EntityPredicate {
   ...
   public static LootContext getLootContext(ServerPlayerEntity player, Entity entity) {
      return (new LootContext.Builder(player.getServerWorld())).withParameter(LootParameters.THIS_ENTITY, entity).withParameter(LootParameters.ORIGIN, player.getPositionVec()).withRandom(player.getRNG()).build(LootParameterSets.ADVANCEMENT_ENTITY);
   }
   ...
   public static class AndPredicate {
      ...
      private static EntityPredicate.AndPredicate fromJSONElement(String name, ConditionArrayParser conditions, @Nullable JsonElement element) {
         if (element != null && element.isJsonArray()) {
            ILootCondition[] ailootcondition = conditions.deserializeConditions(element.getAsJsonArray(), conditions.getId().toString() + "/" + name, LootParameterSets.ADVANCEMENT_ENTITY);
      ...
      public JsonElement serializeConditions(ConditionArraySerializer serializer) {
         return (JsonElement)(this.lootConditions.length == 0 ? JsonNull.INSTANCE : serializer.serialize(this.lootConditions));
      }
