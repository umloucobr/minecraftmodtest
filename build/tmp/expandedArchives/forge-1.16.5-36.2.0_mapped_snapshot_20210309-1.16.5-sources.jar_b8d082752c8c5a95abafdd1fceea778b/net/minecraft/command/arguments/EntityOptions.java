            ...
            if (!map.isEmpty()) {
               p_197457_0_.addFilter((p_197465_1_) -> {
                  ...
                  for(Entry<String, MinMaxBounds.IntBound> entry : map.entrySet()) {
                     ScoreObjective scoreobjective = scoreboard.getObjectiveNullable(entry.getKey());
         ...
         register("predicate", (p_229367_0_) -> {
            ...
            p_229367_0_.addFilter((p_229366_2_) -> {
               if (!(p_229366_2_.world instanceof ServerWorld)) {
                  ...
                  ILootCondition ilootcondition = serverworld.getServer().func_229736_aP_().getCondition(resourcelocation);
                  if (ilootcondition == null) {
                     return false;
                  } else {
                     LootContext lootcontext = (new LootContext.Builder(serverworld)).withParameter(LootParameters.THIS_ENTITY, p_229366_2_).withParameter(LootParameters.ORIGIN, p_229366_2_.getPositionVec()).build(LootParameterSets.SELECTOR);
