...
public class DragonFightManager {
   ...
   private void findAliveCrystals() {
      ...
      for(EndSpikeFeature.EndSpike endspikefeature$endspike : EndSpikeFeature.getSpikes(this.world)) {
   ...
   private void generatePortal(boolean active) {
      ...
      if (this.exitPortalLocation == null) {
         for(this.exitPortalLocation = this.world.getHeight(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, EndPodiumFeature.END_PODIUM_LOCATION).down(); this.world.getBlockState(this.exitPortalLocation).matchesBlock(Blocks.BEDROCK) && this.exitPortalLocation.getY() > this.world.getSeaLevel(); this.exitPortalLocation = this.exitPortalLocation.down()) {
         }
      }
            ...
            for(int i = 0; i < this.portalPattern.getPalmLength(); ++i) {
               for(int j = 0; j < this.portalPattern.getThumbLength(); ++j) {
                  for(int k = 0; k < this.portalPattern.getFingerLength(); ++k) {
                     ...
                     if (cachedblockinfo.getBlockState().matchesBlock(Blocks.BEDROCK) || cachedblockinfo.getBlockState().matchesBlock(Blocks.END_PORTAL)) {
