...
public abstract class WorldCarver<C extends ICarverConfig> extends net.minecraftforge.registries.ForgeRegistryEntry<WorldCarver<?>> {
   ...
   public WorldCarver(Codec<C> codec, int maxHeight) {
      this.maxHeight = maxHeight;
      this.codec = codec.fieldOf("config").xmap(this::func_242761_a, ConfiguredCarver::getConfig).codec();
   }
   ...
   protected boolean canCarveBlock(BlockState state, BlockState aboveState) {
      return this.isCarvable(state) || (state.matchesBlock(Blocks.SAND) || state.matchesBlock(Blocks.GRAVEL)) && !aboveState.getFluidState().isTagged(FluidTags.WATER);
   }
