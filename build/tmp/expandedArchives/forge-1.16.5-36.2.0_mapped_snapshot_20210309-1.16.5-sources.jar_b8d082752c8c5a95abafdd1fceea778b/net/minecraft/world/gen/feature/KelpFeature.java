...
public class KelpFeature extends Feature<NoFeatureConfig> {
   ...
   public boolean generate(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos, NoFeatureConfig config) {
      ...
      if (reader.getBlockState(blockpos).matchesBlock(Blocks.WATER)) {
         ...
         for(int l = 0; l <= k; ++l) {
            if (reader.getBlockState(blockpos).matchesBlock(Blocks.WATER) && reader.getBlockState(blockpos.up()).matchesBlock(Blocks.WATER) && blockstate1.isValidPosition(reader, blockpos)) {
               if (l == k) {
               ...
               if (blockstate.isValidPosition(reader, blockpos1) && !reader.getBlockState(blockpos1.down()).matchesBlock(Blocks.KELP)) {
