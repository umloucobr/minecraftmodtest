   ...
   public void buildSurface(Random random, IChunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, SurfaceBuilderConfig config) {
      ...
      for(int k1 = Math.max(startHeight, (int)d0 + 1); k1 >= 0; --k1) {
         ...
         if (blockstate3.isAir()) {
            k = -1;
         } else if (!blockstate3.matchesBlock(defaultBlock.getBlock())) {
            if (blockstate3.matchesBlock(Blocks.PACKED_ICE) && l <= i1 && k1 > j1) {
            ...
            --k;
            chunkIn.setBlockState(blockpos$mutable, blockstate1, false);
            if (k == 0 && blockstate1.matchesBlock(Blocks.SAND) && j > 1) {
