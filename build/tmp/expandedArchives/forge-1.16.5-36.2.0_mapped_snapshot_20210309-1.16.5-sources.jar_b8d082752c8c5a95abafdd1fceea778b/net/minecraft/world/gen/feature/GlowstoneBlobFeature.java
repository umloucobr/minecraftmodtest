...
public class GlowstoneBlobFeature extends Feature<NoFeatureConfig> {
   ...
   public boolean generate(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos, NoFeatureConfig config) {
      if (!reader.isAirBlock(pos)) {
         ...
         if (!blockstate.matchesBlock(Blocks.NETHERRACK) && !blockstate.matchesBlock(Blocks.BASALT) && !blockstate.matchesBlock(Blocks.BLACKSTONE)) {
            return false;
         } else {
            ...
            for(int i = 0; i < 1500; ++i) {
               ...
               if (reader.getBlockState(blockpos).isAir(reader, blockpos)) {
                  ...
                  for(Direction direction : Direction.values()) {
                     if (reader.getBlockState(blockpos.offset(direction)).matchesBlock(Blocks.GLOWSTONE)) {
                        ++j;
                     }
