...
public class WorldSummary implements Comparable<WorldSummary> {
   ...
   private ITextComponent createDescription() {
      if (this.isLocked()) {
         ...
         IFormattableTextComponent iformattabletextcomponent = (IFormattableTextComponent)(this.isHardcoreModeEnabled() ? (new StringTextComponent("")).appendSibling((new TranslationTextComponent("gameMode.hardcore")).mergeStyle(TextFormatting.DARK_RED)) : new TranslationTextComponent("gameMode." + this.getEnumGameType().getName()));
         if (this.getCheatsEnabled()) {
            iformattabletextcomponent.appendString(", ").appendSibling(new TranslationTextComponent("selectWorld.cheats"));
         }

         IFormattableTextComponent iformattabletextcomponent1 = this.getVersionName();
         IFormattableTextComponent iformattabletextcomponent2 = (new StringTextComponent(", ")).appendSibling(new TranslationTextComponent("selectWorld.version")).appendString(" ");
         if (this.markVersionInList()) {
            iformattabletextcomponent2.appendSibling(iformattabletextcomponent1.mergeStyle(this.askToOpenWorld() ? TextFormatting.RED : TextFormatting.ITALIC));
         } else {
            iformattabletextcomponent2.appendSibling(iformattabletextcomponent1);
         }

         iformattabletextcomponent.appendSibling(iformattabletextcomponent2);
