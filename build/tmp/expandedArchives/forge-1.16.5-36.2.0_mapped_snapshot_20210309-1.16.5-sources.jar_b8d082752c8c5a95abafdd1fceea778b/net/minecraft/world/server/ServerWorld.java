...
public class ServerWorld extends World implements ISeedReader, net.minecraftforge.common.extensions.IForgeWorldServer {
   public static final BlockPos END_SPAWN_AREA = new BlockPos(100, 50, 0);
   ...
   private final ServerChunkProvider serverChunkProvider;
   boolean tickingEntities;
   private final MinecraftServer server;
   private final IServerWorldInfo serverWorldInfo;
   public boolean disableLevelSaving;
   private boolean allPlayersSleeping;
   private int updateEntityTick;
   private final Teleporter worldTeleporter;
   private final ServerTickList<Block> pendingBlockTicks = new ServerTickList<>(this, (block) -> {
      return block == null || block.getDefaultState().isAir();
   }, Registry.BLOCK::getKey, this::tickBlock);
   private final ServerTickList<Fluid> pendingFluidTicks = new ServerTickList<>(this, (fluid) -> {
   ...
   private final List<ISpecialSpawner> specialSpawners;
   @Nullable
   private final DragonFightManager dragonFightManager;
   private final StructureManager structureManager;
   private final boolean shouldBeTicking;
   ...
   public ServerWorld(MinecraftServer server, Executor backgroundExecutor, SaveFormat.LevelSave levelSave, IServerWorldInfo serverWorldInfo, RegistryKey<World> dimension, DimensionType dimensionType, IChunkStatusListener statusListener, ChunkGenerator chunkGenerator, boolean isDebug, long seed, List<ISpecialSpawner> specialSpawners, boolean shouldBeTicking) {
      super(serverWorldInfo, dimension, dimensionType, server::getProfiler, false, isDebug, seed);
      this.shouldBeTicking = shouldBeTicking;
      this.server = server;
      this.specialSpawners = specialSpawners;
      this.serverWorldInfo = serverWorldInfo;
      this.serverChunkProvider = new ServerChunkProvider(this, levelSave, server.getDataFixer(), server.getTemplateManager(), backgroundExecutor, chunkGenerator, server.getPlayerList().getViewDistance(), server.func_230540_aS_(), statusListener, () -> {
         return server.func_241755_D_().getSavedData();
      });
      ...
      }, RaidManager.getName(this.getDimensionType()));
      if (!server.isSinglePlayer()) {
         serverWorldInfo.setGameType(server.getGameType());
      }

      this.structureManager = new StructureManager(this, server.getServerConfiguration().getDimensionGeneratorSettings());
      if (this.getDimensionType().doesHasDragonFight()) {
         this.dragonFightManager = new DragonFightManager(this, server.getServerConfiguration().getDimensionGeneratorSettings().getSeed(), server.getServerConfiguration().getDragonFightData());
      } else {
   ...
   public void setWeather(int clearTime, int weatherTime, boolean isRaining, boolean isThundering) {
      this.serverWorldInfo.setClearWeatherTime(clearTime);
      this.serverWorldInfo.setRainTime(weatherTime);
      this.serverWorldInfo.setThunderTime(weatherTime);
   ...
   public void tick(BooleanSupplier hasTimeLeft) {
      ...
      if (this.getDimensionType().hasSkyLight()) {
         if (this.getGameRules().getBoolean(GameRules.DO_WEATHER_CYCLE)) {
            int i = this.serverWorldInfo.getClearWeatherTime();
            int j = this.serverWorldInfo.getThunderTime();
            int k = this.serverWorldInfo.getRainTime();
            ...
            this.serverWorldInfo.setThunderTime(j);
            this.serverWorldInfo.setRainTime(k);
            this.serverWorldInfo.setClearWeatherTime(i);
            this.serverWorldInfo.setThundering(flag1);
            this.serverWorldInfo.setRaining(flag2);
         }
      ...
      if (this.prevRainingStrength != this.rainingStrength) {
         this.server.getPlayerList().func_232642_a_(new SChangeGameStatePacket(SChangeGameStatePacket.SET_RAIN_STRENGTH, this.rainingStrength), this.getDimensionKey());
      }

      if (this.prevThunderingStrength != this.thunderingStrength) {
         this.server.getPlayerList().func_232642_a_(new SChangeGameStatePacket(SChangeGameStatePacket.SET_THUNDER_STRENGTH, this.thunderingStrength), this.getDimensionKey());
      }
      ...
      if (flag != this.isRaining()) {
         if (flag) {
            this.server.getPlayerList().func_232642_a_(new SChangeGameStatePacket(SChangeGameStatePacket.CLEAR, 0.0F), this.getDimensionKey());
         } else {
            this.server.getPlayerList().func_232642_a_(new SChangeGameStatePacket(SChangeGameStatePacket.RAINING, 0.0F), this.getDimensionKey());
         }
      ...
      if (this.allPlayersSleeping && this.players.stream().noneMatch((player) -> {
         return !player.isSpectator() && !player.isPlayerFullyAsleep();
      })) {
   ...
   protected void tickWorld() {
      if (this.shouldBeTicking) {
         long i = this.worldInfo.getGameTime() + 1L;
         this.serverWorldInfo.setGameTime(i);
         this.serverWorldInfo.getScheduledEvents().run(this.server, i);
   ...
   public void setDayTime(long time) {
      this.serverWorldInfo.setDayTime(time);
   }

   public void updateSpecialSpawners(boolean spawnHostiles, boolean spawnPassives) {
      for(ISpecialSpawner ispecialspawner : this.specialSpawners) {
         ispecialspawner.onUpdate(this, spawnHostiles, spawnPassives);
      }
   ...
   protected BlockPos adjustPosToNearbyEntity(BlockPos pos) {
      ...
      List<LivingEntity> list = this.getEntitiesWithinAABB(LivingEntity.class, axisalignedbb, (entity) -> {
         return entity != null && entity.isAlive() && this.canSeeSky(entity.getPosition());
      });
      if (!list.isEmpty()) {
   ...
   private void resetRainAndThunder() {
      this.serverWorldInfo.setRainTime(0);
      this.serverWorldInfo.setRaining(false);
      this.serverWorldInfo.setThunderTime(0);
      this.serverWorldInfo.setThundering(false);
   }
   ...
   private void tickBlock(NextTickListEntry<Block> blockTickEntry) {
      ...
      if (blockstate.matchesBlock(blockTickEntry.getTarget())) {
   ...
   public void chunkCheck(Entity entityIn) {
      if (entityIn.markLoadedDirty()) {
         ...
         if (!entityIn.addedToChunk || entityIn.chunkCoordX != i || entityIn.chunkCoordY != j || entityIn.chunkCoordZ != k) {
            ...
            if (!entityIn.markPositionDirty() && !this.chunkExists(i, k)) {
               if (entityIn.addedToChunk) {
   ...
   private void saveLevel() {
      if (this.dragonFightManager != null) {
         this.server.getServerConfiguration().setDragonFightData(this.dragonFightManager.write());
      }
   ...
   private boolean hasDuplicateEntity(Entity entityIn) {
      ...
      Entity entity = this.getLoadedOrPendingEntity(uuid);
      if (entity == null) {
         return false;
      } else {
   ...
   @Nullable
   private Entity getLoadedOrPendingEntity(UUID uuid) {
    ...
    */
   public boolean addEntityAndUniquePassengers(Entity entity) {
   ...
   public void removeEntityComplete(Entity entityIn, boolean keepData) {
      if (entityIn.isMultipartEntity()) {
         for(net.minecraftforge.entity.PartEntity<?> enderdragonpartentity : entityIn.getParts()) {
   ...
   public void notifyBlockUpdate(BlockPos pos, BlockState oldState, BlockState newState, int flags) {
      ...
      VoxelShape voxelshape = oldState.getCollisionShapeUncached(this, pos);
      VoxelShape voxelshape1 = newState.getCollisionShapeUncached(this, pos);
   ...
   public ServerChunkProvider getChunkProvider() {
      return this.serverChunkProvider;
   }
   ...
   private boolean fireBlockEvent(BlockEventData event) {
      ...
      return blockstate.matchesBlock(event.getBlock()) ? blockstate.receiveBlockEvent(this, event.getPosition(), event.getEventID(), event.getEventParameter()) : false;
   }
   ...
   @Nullable
   public BlockPos getStructureLocation(Structure<?> structure, BlockPos pos, int radius, boolean skipExistingChunks) {
      return !this.server.getServerConfiguration().getDimensionGeneratorSettings().doesGenerateFeatures() ? null : this.getChunkProvider().getChunkGenerator().func_235956_a_(this, structure, pos, radius, skipExistingChunks);
   }

   @Nullable
   public BlockPos getBiomeLocation(Biome biome, BlockPos pos, int radius, int increment) {
      return this.getChunkProvider().getChunkGenerator().getBiomeProvider().findBiomePosition(pos.getX(), pos.getY(), pos.getZ(), radius, increment, (biome2) -> {
         return biome2 == biome;
      }, this.rand, true);
   }
   ...
   public void setSpawnLocation(BlockPos pos, float angle) {
      ...
      this.getServer().getPlayerList().sendPacketToAllPlayers(new SWorldSpawnChangedPacket(pos, angle));
   }
   ...
   public void onBlockStateChange(BlockPos pos, BlockState blockStateIn, BlockState newState) {
      ...
      if (!Objects.equals(optional, optional1)) {
         ...
         optional1.ifPresent((type) -> {
            this.getServer().execute(() -> {
               this.getPointOfInterestManager().add(blockpos, type);
   ...
   public boolean isVillage(BlockPos pos) {
      return this.doesVillageHaveAllSections(pos, 1);
   }
   ...
   public boolean doesVillageHaveAllSections(BlockPos pos, int sections) {
      if (sections > 6) {
         return false;
      } else {
         return this.sectionsToVillage(SectionPos.from(pos)) <= sections;
      }
   }
   ...
   public void clearBlockEvents(MutableBoundingBox boundingBox) {
      this.blockEventQueue.removeIf((eventData) -> {
         return boundingBox.isVecInside(eventData.getPosition());
      });
   }

   public void updateBlock(BlockPos pos, Block block) {
      if (!this.isDebug()) {
    ...
    */
   public Iterable<Entity> getEntitiesIteratable() {
      return Iterables.unmodifiableIterable(this.entitiesById.values());
   }

   public String toString() {
      return "ServerLevel[" + this.serverWorldInfo.getWorldName() + "]";
   }

   public boolean isFlatWorld() {
      return this.server.getServerConfiguration().getDimensionGeneratorSettings().hasFlatChunkGenerator();
   }
   ...
   public Stream<? extends StructureStart<?>> func_241827_a(SectionPos p_241827_1_, Structure<?> p_241827_2_) {
      return this.getStructureManager().func_235011_a_(p_241827_1_, p_241827_2_);
   }
   ...
   public String func_244521_F() {
      ...
      }), this.getPendingBlockTicks().getSize(), this.getPendingFluidTicks().getSize(), this.getProviderName());
   }
   ...
   public static void setupEndSpawnPlatform(ServerWorld serverWorld) {
      BlockPos blockpos = END_SPAWN_AREA;
      int i = blockpos.getX();
      int j = blockpos.getY() - 2;
      int k = blockpos.getZ();
      BlockPos.getAllInBoxMutable(i - 2, j + 1, k - 2, i + 2, j + 3, k + 2).forEach((p_244430_1_) -> {
         serverWorld.setBlockState(p_244430_1_, Blocks.AIR.getDefaultState());
      });
