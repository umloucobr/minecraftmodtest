...
public class ChunkManager extends ChunkLoader implements ChunkHolder.IPlayerProvider {
   ...
   private final Supplier<DimensionSavedDataManager> dimensionSaveDataGetter;
   ...
   private final IChunkStatusListener statusListener;
   private final ChunkManager.ProxyTicketManager ticketManager;
   private final AtomicInteger loadedChunkCount = new AtomicInteger();
   ...
   public ChunkManager(ServerWorld world, SaveFormat.LevelSave levelSave, DataFixer fixer, TemplateManager templateManager, Executor backgroundExecutor, ThreadTaskExecutor<Runnable> executor, IChunkLightProvider lightProvider, ChunkGenerator p_i232602_8_, IChunkStatusListener statusListener, Supplier<DimensionSavedDataManager> dimensionSaveDataGetter, int viewDistance, boolean isSyncable) {
      super(new File(levelSave.getDimensionFolder(world.getDimensionKey()), "region"), fixer, isSyncable);
      this.templateManager = templateManager;
      this.dimensionDirectory = levelSave.getDimensionFolder(world.getDimensionKey());
      this.world = world;
      this.generator = p_i232602_8_;
      this.mainThread = executor;
      DelegatedTaskExecutor<Runnable> delegatedtaskexecutor = DelegatedTaskExecutor.create(backgroundExecutor, "worldgen");
      ITaskExecutor<Runnable> itaskexecutor = ITaskExecutor.inline("main", executor::enqueue);
      this.statusListener = statusListener;
      DelegatedTaskExecutor<Runnable> delegatedtaskexecutor1 = DelegatedTaskExecutor.create(backgroundExecutor, "light");
      this.sorter = new ChunkTaskPriorityQueueSorter(ImmutableList.of(delegatedtaskexecutor, itaskexecutor, delegatedtaskexecutor1), backgroundExecutor, Integer.MAX_VALUE);
      this.field_219264_r = this.sorter.func_219087_a(delegatedtaskexecutor, false);
      this.field_219265_s = this.sorter.func_219087_a(itaskexecutor, false);
      this.lightManager = new ServerWorldLightManager(lightProvider, this, this.world.getDimensionType().hasSkyLight(), delegatedtaskexecutor1, this.sorter.func_219087_a(delegatedtaskexecutor1, false));
      this.ticketManager = new ChunkManager.ProxyTicketManager(backgroundExecutor, executor);
      this.dimensionSaveDataGetter = dimensionSaveDataGetter;
      this.pointOfInterestManager = new PointOfInterestManager(new File(this.dimensionDirectory, "poi"), fixer, isSyncable);
      ...
      if (oldLevel > MAX_LOADED_LEVEL && newLevel > MAX_LOADED_LEVEL) {
         ...
         if (newLevel <= MAX_LOADED_LEVEL && holder == null) {
            ...
            if (holder != null) {
               ...
               holder = new ChunkHolder(new ChunkPos(chunkPosIn), newLevel, this.lightManager, this.sorter, this);
            }
      ...
      completablefuture.thenAcceptAsync((p_219185_5_) -> {
         ...
         if (completablefuture1 != completablefuture) {
            ...
            if (this.chunksToUnload.remove(chunkPosIn, chunkHolderIn) && p_219185_5_ != null) {
               ...
               this.statusListener.statusChanged(p_219185_5_.getPos(), (ChunkStatus)null);
            }
         ...
         return completablefuture.thenComposeAsync((p_223180_4_) -> {
            ...
            if (!optional.isPresent()) {
               ...
               if (ichunk.getStatus().isAtLeast(chunkStatus)) {
                  ...
                  this.statusListener.statusChanged(chunkpos, chunkStatus);
   ...
   public int func_219174_c() {
      return this.loadedChunkCount.get();
   }
      ...
      if (player.world == this.world) {
         ...
         if (load && !wasLoaded) {
            ...
            if (chunkholder != null) {
               ...
               DebugPacketSender.sendChunkPos(this.world, chunkPosIn);
            }
         }
   ...
   private CompoundNBT loadChunkData(ChunkPos pos) throws IOException {
      ...
      return compoundnbt == null ? null : this.func_235968_a_(this.world.getDimensionKey(), this.dimensionSaveDataGetter, compoundnbt);
   }
   ...
   protected void track(Entity entityIn) {
      if (!(entityIn instanceof net.minecraftforge.entity.PartEntity)) {
   ...
   private void sendChunkData(ServerPlayerEntity player, IPacket<?>[] packetCache, Chunk chunkIn) {
      ...
      DebugPacketSender.sendChunkPos(this.world, chunkIn.getPos());
   ...
   class EntityTracker {
      ...
      public EntityTracker(Entity entity, int range, int updateFrequency, boolean sendVelocityUpdates) {
