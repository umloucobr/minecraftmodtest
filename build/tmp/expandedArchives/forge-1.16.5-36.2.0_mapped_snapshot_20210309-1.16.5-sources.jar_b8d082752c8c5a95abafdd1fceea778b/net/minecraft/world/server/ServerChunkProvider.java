...
public class ServerChunkProvider extends AbstractChunkProvider {
   ...
   @Nullable
   private WorldEntitySpawner.EntityDensityManager entityDensityManager;

   public ServerChunkProvider(ServerWorld world, SaveFormat.LevelSave levelSave, DataFixer fixer, TemplateManager templateManager, Executor backgroundExecutor, ChunkGenerator p_i232603_6_, int viewDistance, boolean isSyncable, IChunkStatusListener statusListener, Supplier<DimensionSavedDataManager> dimensionSaveDataGetter) {
      this.world = world;
      this.executor = new ServerChunkProvider.ChunkExecutor(world);
      this.generator = p_i232603_6_;
      this.mainThread = Thread.currentThread();
      File file1 = levelSave.getDimensionFolder(world.getDimensionKey());
      File file2 = new File(file1, "data");
      file2.mkdirs();
      this.savedData = new DimensionSavedDataManager(file2, fixer);
      this.chunkManager = new ChunkManager(world, levelSave, fixer, templateManager, backgroundExecutor, this.executor, this, this.getChunkGenerator(), statusListener, dimensionSaveDataGetter, viewDistance, isSyncable);
   ...
   private void func_225315_a(long chunkPos, IChunk p_225315_3_, ChunkStatus p_225315_4_) {
      for(int i = 3; i > 0; --i) {
      ...
      this.recentPositions[0] = chunkPos;
   ...
   private void tickChunks() {
      ...
      if (!flag) {
         ...
         WorldEntitySpawner.EntityDensityManager worldentityspawner$entitydensitymanager = WorldEntitySpawner.func_234964_a_(l, this.world.getEntitiesIteratable(), this::func_241098_a_);
         this.entityDensityManager = worldentityspawner$entitydensitymanager;
         this.world.getProfiler().endSection();
         ...
         if (flag1) {
            this.world.updateSpecialSpawners(this.spawnHostiles, this.spawnPassives);
         }
   ...
   public <T> void registerTickingTicket(TicketType<T> type, ChunkPos pos, int distance, T value) {
      this.ticketManager.registerTicking(type, pos, distance, value);
   }

   public <T> void releaseTickingTicket(TicketType<T> type, ChunkPos pos, int distance, T value) {
      this.ticketManager.releaseTicking(type, pos, distance, value);
   }
   ...
   public WorldEntitySpawner.EntityDensityManager func_241101_k_() {
      return this.entityDensityManager;
   }
