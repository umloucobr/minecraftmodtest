...
public class BiomeGenerationSettings {
   ...
   public static final MapCodec<BiomeGenerationSettings> CODEC = RecordCodecBuilder.mapCodec((builder) -> {
      ...
      }), Codec.simpleMap(GenerationStage.Carving.CODEC, ConfiguredCarver.field_242759_c.promotePartial(Util.prefixString("Carver: ", LOGGER::error)), IStringSerializable.createKeyable(GenerationStage.Carving.values())).fieldOf("carvers").forGetter((settings) -> {
         return settings.carvers;
      }), ConfiguredFeature.field_242764_c.promotePartial(Util.prefixString("Feature: ", LOGGER::error)).listOf().fieldOf("features").forGetter((settings) -> {
         return settings.features;
      }), StructureFeature.field_242770_c.promotePartial(Util.prefixString("Structure start: ", LOGGER::error)).fieldOf("starts").forGetter((settings) -> {
   ...
   private BiomeGenerationSettings(Supplier<ConfiguredSurfaceBuilder<?>> surfaceBuilder, Map<GenerationStage.Carving, List<Supplier<ConfiguredCarver<?>>>> carversIn, List<List<Supplier<ConfiguredFeature<?, ?>>>> features, List<Supplier<StructureFeature<?, ?>>> structures) {
      ...
      this.flowerFeatures = features.stream().flatMap(Collection::stream).map(Supplier::get).flatMap(ConfiguredFeature::getConfiguredFeatures).filter((configuredFeature) -> {
