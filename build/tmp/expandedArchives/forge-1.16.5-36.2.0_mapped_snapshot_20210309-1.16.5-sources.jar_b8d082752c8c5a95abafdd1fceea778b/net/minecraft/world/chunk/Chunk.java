...
public class Chunk extends net.minecraftforge.common.capabilities.CapabilityProvider<Chunk> implements IChunk, net.minecraftforge.common.extensions.IForgeChunk {
   ...
   public BlockState setBlockState(BlockPos pos, BlockState state, boolean isMoving) {
      ...
      if (blockstate == state) {
         ...
         if (!chunksection.getBlockState(i, j & 15, k).matchesBlock(block)) {
            return null;
         } else {
            ...
            if (!this.world.isRemote && !this.world.captureBlockSnapshots) {
   ...
   public Stream<BlockPos> getLightSources() {
      return StreamSupport.stream(BlockPos.getAllInBoxMutable(this.pos.getXStart(), 0, this.pos.getZStart(), this.pos.getXEnd(), 255, this.pos.getZEnd()).spliterator(), false).filter((pos) -> {
         return this.getBlockState(pos).getLightValue(getWorld(), pos) != 0;
      });
   }
