...
public class RaidManager extends WorldSavedData {
   ...
   public RaidManager(ServerWorld world) {
      super(getName(world.getDimensionType()));
      this.world = world;
   ...
   public Raid badOmenTick(ServerPlayerEntity player) {
      if (player.isSpectator()) {
         ...
         if (!dimensiontype.isHasRaids()) {
            ...
            Raid raid = this.findOrCreateRaid(player.getServerWorld(), blockpos1);
            boolean flag = false;
            if (!raid.isStarted()) {
   ...
   private Raid findOrCreateRaid(ServerWorld world, BlockPos pos) {
      Raid raid = world.findRaid(pos);
      return raid != null ? raid : new Raid(this.incrementNextId(), world, pos);
   }
   ...
   public static String getName(DimensionType p_234620_0_) {
