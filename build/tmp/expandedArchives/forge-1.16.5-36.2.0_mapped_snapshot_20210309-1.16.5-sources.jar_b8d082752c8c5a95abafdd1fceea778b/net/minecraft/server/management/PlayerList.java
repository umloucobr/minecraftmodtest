...
public abstract class PlayerList {
   ...
   public ITextComponent canPlayerLogin(SocketAddress p_206258_1_, GameProfile p_206258_2_) {
      if (this.bannedPlayers.isBanned(p_206258_2_)) {
         ...
         if (profilebanentry.getBanEndDate() != null) {
            iformattabletextcomponent1.appendSibling(new TranslationTextComponent("multiplayer.disconnect.banned.expiration", DATE_FORMAT.format(profilebanentry.getBanEndDate())));
         }
         ...
         if (ipbanentry.getBanEndDate() != null) {
            iformattabletextcomponent.appendSibling(new TranslationTextComponent("multiplayer.disconnect.banned_ip.expiration", DATE_FORMAT.format(ipbanentry.getBanEndDate())));
         }
   ...
   public ServerPlayerEntity func_232644_a_(ServerPlayerEntity p_232644_1_, boolean p_232644_2_) {
      ...
      if (optional.isPresent()) {
         ...
         boolean flag1 = blockstate.matchesBlock(Blocks.RESPAWN_ANCHOR);
         ...
         serverplayerentity.connection.sendPacket(new SChangeGameStatePacket(SChangeGameStatePacket.SPAWN_NOT_VALID, 0.0F));
      }
      ...
      serverplayerentity.connection.sendPacket(new SWorldSpawnChangedPacket(serverworld1.getSpawnPoint(), serverworld1.getSpawnAngle()));
   ...
   public void sendWorldInfo(ServerPlayerEntity playerIn, ServerWorld worldIn) {
      ...
      playerIn.connection.sendPacket(new SWorldSpawnChangedPacket(worldIn.getSpawnPoint(), worldIn.getSpawnAngle()));
      if (worldIn.isRaining()) {
         playerIn.connection.sendPacket(new SChangeGameStatePacket(SChangeGameStatePacket.RAINING, 0.0F));
         playerIn.connection.sendPacket(new SChangeGameStatePacket(SChangeGameStatePacket.SET_RAIN_STRENGTH, worldIn.getRainStrength(1.0F)));
         playerIn.connection.sendPacket(new SChangeGameStatePacket(SChangeGameStatePacket.SET_THUNDER_STRENGTH, worldIn.getThunderStrength(1.0F)));
      }
