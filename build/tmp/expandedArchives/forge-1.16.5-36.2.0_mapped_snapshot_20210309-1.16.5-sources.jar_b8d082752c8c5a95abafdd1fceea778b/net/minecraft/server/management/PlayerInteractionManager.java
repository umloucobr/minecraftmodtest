...
public class PlayerInteractionManager {
      ...
      if (exp == -1) {
         ...
         if ((block instanceof CommandBlockBlock || block instanceof StructureBlock || block instanceof JigsawBlock) && !this.player.canUseCommandBlock()) {
            ...
            if (this.isCreative()) {
               ...
               boolean flag1 = blockstate.canHarvestBlock(this.world, pos, this.player); // previously player.hasCorrectToolForDrops(blockstate)
   ...
   public ActionResultType func_219441_a(ServerPlayerEntity playerIn, World worldIn, ItemStack stackIn, Hand handIn, BlockRayTraceResult blockRaytraceResultIn) {
      ...
      net.minecraftforge.event.entity.player.PlayerInteractEvent.RightClickBlock event = net.minecraftforge.common.ForgeHooks.onRightClickBlock(playerIn, handIn, blockpos, blockRaytraceResultIn);
      ...
      if (this.gameType == GameType.SPECTATOR) {
         ...
         if (event.getUseItem() == net.minecraftforge.eventbus.api.Event.Result.ALLOW || (!stackIn.isEmpty() && !playerIn.getCooldownTracker().hasCooldown(stackIn.getItem()))) {
