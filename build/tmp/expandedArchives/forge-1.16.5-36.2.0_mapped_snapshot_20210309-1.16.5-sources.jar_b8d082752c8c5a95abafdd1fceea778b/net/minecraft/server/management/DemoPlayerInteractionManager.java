...
public class DemoPlayerInteractionManager extends PlayerInteractionManager {
   ...
   public void tick() {
      ...
      if (!this.displayedIntro && this.gameModeTicks > 20) {
         ...
         this.player.connection.sendPacket(new SChangeGameStatePacket(SChangeGameStatePacket.CHANGE_SETTINGS, 0.0F));
      }
      ...
      if (i % 24000L == 500L) {
         if (j <= 6L) {
            if (j == 6L) {
               this.player.connection.sendPacket(new SChangeGameStatePacket(SChangeGameStatePacket.CHANGE_SETTINGS, 104.0F));
            } else {
         ...
         if (i == 100L) {
            this.player.connection.sendPacket(new SChangeGameStatePacket(SChangeGameStatePacket.CHANGE_SETTINGS, 101.0F));
         } else if (i == 175L) {
            this.player.connection.sendPacket(new SChangeGameStatePacket(SChangeGameStatePacket.CHANGE_SETTINGS, 102.0F));
         } else if (i == 250L) {
            this.player.connection.sendPacket(new SChangeGameStatePacket(SChangeGameStatePacket.CHANGE_SETTINGS, 103.0F));
         }
